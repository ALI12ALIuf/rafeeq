...
const SecureChatSystem = {
    ...
    async compressVideo(file) {
        return new Promise((resolve) => {
            ...
            video.onloadedmetadata = () => { URL.revokeObjectURL(video.src); let width = video.videoWidth, height = video.videoHeight; if (height > 480) { width *= 480 / height; height = 480; } canvas.width = Math.round(width); canvas.height = Math.round(height); const stream = canvas.captureStream(30); const mediaRecorder = new MediaRecorder(stream, { mimeType: 'video/webm;codecs=vp8', videoBitsPerSecond: 300000 }); const chunks = []; mediaRecorder.ondataavailable = e => chunks.push(e.data); mediaRecorder.onstop = () => resolve(new Blob(chunks, { type: 'video/webm' })); video.currentTime = 0; video.play(); mediaRecorder.start(); setTimeout(() => { mediaRecorder.stop(); video.pause(); }, Math.min(video.duration * 1000, 1800000)); };
...
const CallSystem = {
    ...
    async ensureDataChannel(calleeId) {
        if (this.dc && this.dc.readyState === 'open') {
            try { this.dc.send(JSON.stringify({ type: 'ping' })); console.log('✅ القناة موجودة وشغالة'); return; }
            catch(e) { console.log('⚠️ القناة موجودة لكن ما تستجيب، إعادة بناء'); this.dc.close(); this.dc = null; }
        }
    ...
    async startCall(calleeId, callType = 'video') {
        if (!window.auth?.currentUser || this.isInCall) return; this.isInCall = true;
        try {
    ...
    setupDataChannel(channel) {
        channel.onmessage = e => {
            try {
                ...
                if (msg.type === 'pong') { console.log('💓 نبض - القناة شغالة'); return; }
                if (msg.chunk !== undefined) {
                    ...
                    this.incomingChunks[msg.id][msg.chunk] = msg.data; this.incomingFileInfo[msg.id].received++;
        ...
        channel.onopen = () => { console.log('📡 Data Channel مفتوح'); channel.heartbeat = setInterval(() => { if (channel.readyState === 'open') channel.send(JSON.stringify({ type: 'ping' })); else clearInterval(channel.heartbeat); }, 10000); };
        channel.onclose = () => { console.log('⚠️ Data Channel انغلق'); if (channel.heartbeat) clearInterval(channel.heartbeat); if (ChatSystem.currentChat && ChatSystem.friendOnline) { console.log('🔄 محاولة إعادة الاتصال...'); setTimeout(() => CallSystem.ensureDataChannel(ChatSystem.currentChat), 1000); } };
    },
    
    async sendFileDirect(file, type) {
        if (!this.dc || this.dc.readyState !== 'open') { console.warn('⚠️ القناة ليست مفتوحة، جاري强制 إعادة الاتصال...'); if (ChatSystem.currentChat) { await this.ensureDataChannel(ChatSystem.currentChat); await new Promise(r => setTimeout(r, 1000)); } if (!this.dc || this.dc.readyState !== 'open') { console.error('❌ فشل بناء قناة البيانات'); return false; } }
        try {
            let b64; if (type === 'image') { const comp = await SecureChatSystem.compressImage(file); b64 = await SecureChatSystem.fileToBase64(comp); } else if (type === 'video') { const comp = await SecureChatSystem.compressVideo(file); b64 = await SecureChatSystem.fileToBase64(comp); } else { b64 = await SecureChatSystem.fileToBase64(file); }
            const chunkSize = 16000; const totalChunks = Math.ceil(b64.length / chunkSize); const fileId = Date.now().toString();
            console.log(`📤 بدء إرسال ${type}: ${totalChunks} أجزاء`);
            for (let i = 0; i < totalChunks; i++) { if (this.dc.readyState !== 'open') return false; this.dc.send(JSON.stringify({ type, data: b64.substring(i * chunkSize, (i + 1) * chunkSize), chunk: i, total: totalChunks, id: fileId, fileName: file.name })); await new Promise(r => setTimeout(r, 80)); }
            console.log('✅ تم إرسال جميع الأجزاء'); return true;
    ...
    async sendSignal(calleeId, data) { /* ... بدون تغيير ... */ },
...
const ChatSystem = {
    ...
    openChat(friendId, friendName, friendAvatar) {
        ...
        this.displayMessages(friendId); PresenceSystem.watchFriend(friendId);
        let attempts = 0;
        const tryConnect = () => { if (this.friendOnline && (!CallSystem.dc || CallSystem.dc.readyState !== 'open')) { console.log(`🔄 محاولة بناء القناة (${attempts + 1}/3)...`); CallSystem.ensureDataChannel(friendId); attempts++; if (attempts < 3) setTimeout(tryConnect, 1500); } };
    ...
    updateFriendStatus(friendId, isOnline) {
        ...
        this.friendOnline = isOnline; if (isOnline) setTimeout(() => CallSystem.ensureDataChannel(friendId), 500);
    ...
    updateAttachmentButtons(isOnline) { const btns = document.querySelectorAll('#attachmentMenu button[data-dc]'); btns.forEach(btn => { if (isOnline) { btn.classList.remove('locked'); btn.title = ''; } else { btn.classList.add('locked'); btn.title = 'غير متاح - المستخدم غير متصل'; } }); },
    displayMessages(friendId) { const c = document.getElementById('messagesContainer'); if (!c) return; c.innerHTML = ''; (this.messages[friendId] || []).forEach(m => this.displayMessage(m)); },
    displayMessage(msg) { /* ... بدون تغيير ... */ },
    async sendMessage(text) { /* ... بدون تغيير ... */ },
    async sendImage(file) { if (!this.currentChat) return; if (this.friendOnline && CallSystem.dc && CallSystem.dc.readyState === 'open') { await CallSystem.sendFileDirect(file, 'image'); const comp = await SecureChatSystem.compressImage(file); const b64 = await SecureChatSystem.fileToBase64(comp); this.saveMessage(this.currentChat, { id: Date.now().toString(), type: 'image', data: b64, sender: 'me', time: new Date().toISOString(), status: 'sent' }); this.displayMessage({ id: Date.now().toString(), type: 'image', data: b64, sender: 'me', time: new Date().toISOString(), status: 'sent' }); } },
    async sendVideoFile(file) { if (!this.currentChat) return; if (this.friendOnline && CallSystem.dc && CallSystem.dc.readyState === 'open') { await CallSystem.sendFileDirect(file, 'video'); const b64 = await SecureChatSystem.fileToBase64(await SecureChatSystem.compressVideo(file)); this.saveMessage(this.currentChat, { id: Date.now().toString(), type: 'video', data: b64, sender: 'me', time: new Date().toISOString(), status: 'sent' }); this.displayMessage({ id: Date.now().toString(), type: 'video', data: b64, sender: 'me', time: new Date().toISOString(), status: 'sent' }); } },
    async sendFile(file) { if (!this.currentChat) return; if (this.friendOnline && CallSystem.dc && CallSystem.dc.readyState === 'open') { await CallSystem.sendFileDirect(file, 'file'); const b64 = await SecureChatSystem.fileToBase64(file); this.saveMessage(this.currentChat, { id: Date.now().toString(), type: 'file', data: b64, fileName: file.name, sender: 'me', time: new Date().toISOString(), status: 'sent' }); this.displayMessage({ id: Date.now().toString(), type: 'file', data: b64, fileName: file.name, sender: 'me', time: new Date().toISOString(), status: 'sent' }); } },
    async sendVoiceNote(audioBlob) { if (!this.currentChat) return; if (this.friendOnline && CallSystem.dc && CallSystem.dc.readyState === 'open') { await CallSystem.sendFileDirect(audioBlob, 'voice'); const b64 = await SecureChatSystem.fileToBase64(audioBlob); this.saveMessage(this.currentChat, { id: Date.now().toString(), type: 'voice', data: b64, sender: 'me', time: new Date().toISOString(), status: 'sent' }); this.displayMessage({ id: Date.now().toString(), type: 'voice', data: b64, sender: 'me', time: new Date().toISOString(), status: 'sent' }); } },
    async shareLocationDirect() { if (!this.currentChat) return; if (this.friendOnline && CallSystem.dc && CallSystem.dc.readyState === 'open') { navigator.geolocation.getCurrentPosition(p => { const locMsg = `📍 موقعي: https://www.google.com/maps?q=${p.coords.latitude},${p.coords.longitude}`; CallSystem.dc.send(JSON.stringify({ type: 'location', data: locMsg, id: Date.now().toString() })); this.displayMessage({ id: Date.now().toString(), type: 'text', text: locMsg, sender: 'me', time: new Date().toISOString(), status: 'sent' }); this.saveMessage(this.currentChat, { id: Date.now().toString(), type: 'text', text: locMsg, sender: 'me', time: new Date().toISOString(), status: 'sent' }); }); } },
    ...
    closeChat() { document.body.classList.remove('conversation-open'); document.getElementById('conversationPage').style.display = 'none'; document.querySelector('.chat-page').style.display = 'block'; PresenceSystem.stopAll(); if (CallSystem.dc) { clearInterval(CallSystem.dc.heartbeat); CallSystem.dc.close(); CallSystem.dc = null; } if (CallSystem.pc) { CallSystem.pc.close(); CallSystem.pc = null; } this.currentChat = null; this.friendOnline = false; },
...
window.diagnose = async function() {
    let log = '🔍 تشخيص الاتصال:\n\n';
    log += '1. المستخدم: ' + (window.auth?.currentUser ? '✅ مسجل' : '❌ غير مسجل') + '\n';
    log += '2. المحادثة الحالية: ' + (ChatSystem.currentChat || '❌ لا يوجد') + '\n';
    log += '3. الصديق متصل: ' + (ChatSystem.friendOnline ? '🟢 نعم' : '🔴 لا') + '\n';
    if (CallSystem.dc) {
        log += '4. Data Channel: ' + CallSystem.dc.readyState;
        if (CallSystem.dc.readyState === 'open') {
            log += ' ✅\n   ↳ اختبار ping: ';
            try { CallSystem.dc.send(JSON.stringify({ type: 'ping' })); log += '✅ تم الإرسال'; } catch(e) { log += '❌ ' + e.message; }
        } else { log += ' ⚠️'; }
    } else { log += '4. Data Channel: ❌ غير موجود'; }
    log += '\n5. Peer Connection: ' + (CallSystem.pc ? CallSystem.pc.connectionState : '❌ غير موجود') + '\n';
    try { const doc = await window.db.collection('users').doc(window.auth.currentUser.uid).get(); log += '6. publicKey: ' + (doc.data().publicKey ? '✅ موجود' : '❌ فاضي') + '\n'; } catch(e) { log += '6. Firebase: ❌ ' + e.message + '\n'; }
    log += '7. privateKey: ' + (localStorage.getItem('enc_private_key') ? '✅ موجود' : '❌ فاضي') + '\n';
    alert(log); console.log(log); return log;
};
window.forceConnect = async function() {
    if (!ChatSystem.currentChat) return alert('❌ لا توجد محادثة مفتوحة');
    ...
    alert('🔄 جاري بناء قناة جديدة...');
    try { await CallSystem.ensureDataChannel(ChatSystem.currentChat); setTimeout(async () => { const state = CallSystem.dc ? CallSystem.dc.readyState : 'لا يوجد'; alert('📡 نتيجة: ' + state); if (CallSystem.dc && CallSystem.dc.readyState === 'open') { try { CallSystem.dc.send(JSON.stringify({ type: 'ping' })); alert('✅ القناة شغالة وتم إرسال ping'); } catch(e) { alert('⚠️ القناة open لكن الإرسال فشل: ' + e.message); } } }, 2000); } catch(e) { alert('❌ فشل: ' + e.message); }
};
setTimeout(() => { const btn = document.createElement('button'); btn.textContent = '🔍'; btn.style.cssText = 'position:fixed;bottom:60px;right:10px;z-index:99999;background:#333;color:white;border:1px solid #555;padding:8px;border-radius:50%;font-size:14px;cursor:pointer;width:40px;height:40px;'; btn.onclick = () => window.diagnose(); document.body.appendChild(btn); }, 2000);
setTimeout(() => { const btn2 = document.createElement('button'); btn2.textContent = '🔧'; btn2.style.cssText = 'position:fixed;bottom:60px;right:55px;z-index:99999;background:#2196F3;color:white;border:1px solid #1976D2;padding:8px;border-radius:50%;font-size:14px;cursor:pointer;width:40px;height:40px;'; btn2.onclick = () => window.forceConnect(); document.body.appendChild(btn2); }, 2000);
console.log('🔍 كود التشخيص جاهز. اضغط 🔍 للتشخيص و 🔧 للاتصال القسري');
