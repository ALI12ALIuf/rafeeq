...
const ChatSystem = {
    ...
    async getSharedSecret(friendId) {
        ...
        // ✅ التأكد من وجود keyPair للمستخدم الحالي قبل أي شيء
        const currentUserId = window.auth.currentUser?.uid;
        if (!currentUserId) return null;
        
        if (!window.cryptoSystem.keyPairs.has(currentUserId)) {
            console.log('🔄 Loading existing key pair from storage for user:', currentUserId);
            const loadedKeyPair = await window.cryptoSystem.getOrCreateKeyPair(currentUserId);
            if (loadedKeyPair) {
                window.cryptoSystem.keyPairs.set(currentUserId, loadedKeyPair);
                console.log('✅ Key pair loaded successfully');
            } else {
                console.warn('⚠️ Failed to load key pair');
                return null;
            }
        }
        ...
        try {
            ...
            let myKeyPair = window.cryptoSystem.keyPairs.get(currentUserId);
