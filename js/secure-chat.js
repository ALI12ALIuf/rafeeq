...
const CallSystem = {
    ...
    showIncomingCall(callerId, callData, sessionId) {
        ...
        // الحصول على اسم المتصل
        let contactName = 'مستخدم';
        let contactAvatar = '👤';
        
        // محاولة الحصول على اسم المتصل من التخزين المحلي أو من DOM
        const usersStr = localStorage.getItem('rafeeq_users');
        if (usersStr) {
            try {
                const users = JSON.parse(usersStr);
                const foundUser = users.find(u => u.id === callerId);
                if (foundUser) {
                    contactName = foundUser.name || foundUser.displayName || 'مستخدم';
    ...
    async handleSignaling(data, fromUserId) {
        ...
        // ✅ تجاهل العرض بدون sessionId (قديم)
        if (data.sdp && data.sdp.type === 'offer' && !data.sessionId) {
