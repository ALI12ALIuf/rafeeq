...
const SecureChatSystem = {
    ...
    async processReceivedMessage(msg) {
        try {
            ...
            // ✅ معالجة رفض المكالمة (مع دعم sessionId)
            else if (msg.package.type === 'webrtc_reject') {
                try {
                    const decryptedReject = await this.decryptData(msg.package.data, sharedKey);
                    const rejectData = JSON.parse(decryptedReject);
                    console.log('📞 استلام رفض مكالمة من:', msg.from, 'الجلسة:', rejectData.sessionId);
                    await CallSystem.handleReject(rejectData.sessionId);
                } catch(e) {
