    ...
    async processReceivedMessage(msg) {
        try {
            ...
            if (msg.package.type === 'text') { 
                ...
                const displayMsg = { id: msg.package.id, type: 'text', text: decryptedText, sender: 'friend', time: new Date().toISOString() };
                ChatSystem.saveMessage(msg.from, displayMsg); 
                // ✅ إصلاح: إضافة الرسالة للواجهة فقط بدون إعادة رسم المحادثة بالكامل (الذي كان يمسح الوسائط)
                if (ChatSystem.currentChat === msg.from) ChatSystem.displayMessage(displayMsg);
