...
const DownloadManager = {
    ...
    downloadBlob(blob, filename) {
        // ✅ كل عملية تحميل مستقلة تماماً - لا يؤثر تحميل على آخر
        ...
        try {
            ...
            // ✅ تأخير أطول قبل إلغاء الـ URL لضمان اكتمال التحميل
            setTimeout(() => {
            ...
            console.warn('⚠️ طريقة التنزيل الأولى فشلت، جاري المحاولة بطريقة بديلة...');
            try {
    ...
    if (fileData.package && fileData.package.data) {
        ...
        downloadBtn.onclick = function(e) {
            ...
            try {
                ...
                DownloadManager.showNotification('❌ فشل التحميل', 'error');
            } finally {
                // ✅ دائماً نُعيد الزر بعد ثانية واحدة بغض النظر عن النتيجة
                setTimeout(() => {
