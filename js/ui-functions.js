...
const MediaDownloader = {
    ...
    async registerSW() {
        ...
        try {
            const reg = await navigator.serviceWorker.register('/rafeeq/sw.js', { scope: '/rafeeq/' });
            await navigator.serviceWorker.ready;
            console.log('✅ Service Worker جاهز');
            return true;
        } catch(e) {
            console.warn('⚠️ SW فشل التسجيل:', e);
            return false;
        }
    },

    // ✅ التنزيل عبر Service Worker - يعمل كل مرة على Chrome بدون قيود
    async downloadViaSW(dataUrl, fileName, mimeType) {
        const sw = navigator.serviceWorker?.controller;
        if (!sw) throw new Error('SW غير نشط');
        ...
        await new Promise((resolve, reject) => {
            ...
            channel.port1.onmessage = e => {
                if (e.data.type === 'FILE_READY') resolve();
                else reject(new Error('SW لم يرد'));
            };
            sw.postMessage({ type: 'STORE_FILE', id, fileName, mimeType, data: dataUrl }, [channel.port2]);
            setTimeout(() => reject(new Error('SW timeout')), 5000);
        });

        // فتح رابط https حقيقي = Chrome يسمح بالتنزيل بدون قيود
        const dlUrl = `/rafeeq/_dl/?id=${id}`;
        ...
        console.log('✅ تنزيل عبر SW:', fileName);
    },

    // Fallback: تنزيل مباشر إذا SW غير متوفر
    downloadDirect(dataUrl, fileName, mimeType) {
        ...
        const a = document.createElement('a');
        a.href = url; a.download = fileName || 'ملف'; a.style.display = 'none';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
    ...
    createButton(msgId, fileName, mimeType, dataUrl) {
        ...
        btn.addEventListener('click', async function(e) {
            ...
            try {
                ...
                if (swActive) {
                    // ✅ الطريقة الأفضل - Service Worker
                    await MediaDownloader.downloadViaSW(btn._dataUrl, btn._fileName, btn._mimeType);
                } else {
                    // Fallback
                    MediaDownloader.downloadDirect(btn._dataUrl, btn._fileName, btn._mimeType);
                }
                btn.innerHTML = '<i class="fas fa-check"></i>';
            } catch(err) {
                console.error('❌ فشل التنزيل:', err);
                // محاولة أخيرة بالطريقة المباشرة
                try { MediaDownloader.downloadDirect(btn._dataUrl, btn._fileName, btn._mimeType); } catch(e2) {}
                btn.innerHTML = '<i class="fas fa-download"></i>';
            }
