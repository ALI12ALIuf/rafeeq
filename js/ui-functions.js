    ...
    async save(id, dataUrl, fileName, mimeType) {
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                tx.oncomplete = () => { console.log('✅ MediaDB حفظ:', id, fileName); resolve(true); };
...
const MediaDownloader = {
    createButton(msgId, fileName, mimeType) {
        ...
        // ✅ inline style لضمان الظهور والتفاعل حتى لو CSS خارجي تأخر
        btn.style.cssText = `
        ...
        btn.addEventListener('click', async function(e) {
            ...
            console.log('🔽 ضغط تحميل - id:', id, 'name:', fileName);
            try {
                const record = await MediaDB.get(id);
                console.log('📦 قراءة IndexedDB:', record ? 'موجود (' + record.dataUrl.length + ' chars)' : 'غير موجود!');
                if (!record || !record.dataUrl) {
                    console.error('❌ البيانات غير موجودة في IndexedDB:', id);
                    btn.innerHTML = '❌';
    ...
    triggerDownload(dataUrl, fileName, mimeType) {
        try {
            ...
            console.log('✅ تحميل بدأ:', fileName);
        } catch(err) {
            console.error('❌ triggerDownload فشل:', err);
