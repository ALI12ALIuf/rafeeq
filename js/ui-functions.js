...
// ==================== أداة تشخيص المشاكل على الهاتف ====================
(function diagnoseOnScreen() {
    // إنشاء لوحة تشخيص على الشاشة
    ...
    console.log = function(...args) {
        originalLog(...args);
        addLog(args.map(a => typeof a === 'object' ? JSON.stringify(a).slice(0, 80) : String(a)).join(' '));
    };
    
    console.error = function(...args) {
        originalError(...args);
        addLog('❌ ERROR: ' + args.map(a => String(a)).join(' '), true);
    };
    
    // ✅ 1. التحقق من وجود CallSystem
    setTimeout(() => {
        if (typeof CallSystem !== 'undefined') {
            addLog('✅ CallSystem موجود');
        } else {
            addLog('❌ CallSystem غير موجود (مشكلة في التحميل)', true);
        }
        
        if (typeof ChatSystem !== 'undefined') {
            addLog('✅ ChatSystem موجود');
        } else {
            addLog('❌ ChatSystem غير موجود', true);
        }
    }, 1000);
    
    // ✅ 2. مراقبة أخطاء الـ JavaScript
    window.addEventListener('error', (e) => {
        addLog('🔥 خطأ عام: ' + e.message + ' في ' + e.filename, true);
    });
    
    window.addEventListener('unhandledrejection', (e) => {
        addLog('🔥 Promise خطأ: ' + e.reason, true);
    });
    ...
    if (backBtn) {
        backBtn.addEventListener('click', () => {
            addLog('🔘 تم الضغط على زر الرجوع');
        });
    } else {
        addLog('⚠️ زر الرجوع غير موجود في الصفحة', true);
    }
    
    // ✅ 4. مراقبة closeConversation
    if (typeof closeConversation === 'function') {
        addLog('✅ closeConversation موجودة');
    } else {
        addLog('⚠️ closeConversation غير معرفة', true);
    }
    
    addLog('🔍 نظام التشخيص يعمل...');
})();
