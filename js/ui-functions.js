...
window._pageStack = [];
window._unreadCounts = {};

function pushPage(pageType, pageId) { window._pageStack.push({ type: pageType, id: pageId }); }
function popPage() { return window._pageStack.length > 0 ? window._pageStack.pop() : null; }
function clearStack() { window._pageStack = []; }

// ========== حفظ وتحميل العدادات ==========
function saveUnreadCounts() { try { sessionStorage.setItem('_unreadCounts', JSON.stringify(window._unreadCounts)); } catch(e) {} }
function loadUnreadCounts() { try { const saved = sessionStorage.getItem('_unreadCounts'); if (saved) window._unreadCounts = JSON.parse(saved); } catch(e) { window._unreadCounts = {}; } }
    ...
    try { 
        ...
        for (const fid of friends) { 
            try { 
                ...
                if (fdoc.exists) { 
                    const f = fdoc.data(); 
                    const key = `chat_${fid}`; 
                    let lm = 'اضغط لبدء المحادثة', lt = '', lastTime = 0; 
                    try { 
                        const h = JSON.parse(localStorage.getItem(key)) || []; 
                        if (h.length > 0) { 
                            const l = h[h.length - 1]; 
                            lastTime = new Date(l.time).getTime();
        ...
        for (const item of chatItems) {
            ...
            html += `<div class="chat-item${unreadClass}" onclick="openChat('${item.fid}')">
                ...
                <div class="chat-info">
                    <h4>${item.f.name || 'مستخدم'}</h4>
                    <p class="last-message">${item.lm}</p>
                </div>
                <div class="chat-meta">
...
window.closeConversation = () => { CallSystem.endCall(); ChatSystem.closeChat(); setTimeout(() => { const lastPage = popPage(); document.querySelectorAll('.page').forEach(p => { p.classList.remove('active'); p.style.display = 'none'; }); document.querySelectorAll('.profile-subpage').forEach(s => s.style.display = 'none'); document.body.classList.remove('profile-subpage-open'); if (lastPage && lastPage.type === 'subpage') { document.body.classList.add('profile-subpage-open'); document.querySelector('.profile-page').style.display = 'none'; if (lastPage.id && document.getElementById(lastPage.id)) document.getElementById(lastPage.id).style.display = 'block'; document.querySelectorAll('.nav-item').forEach(n => { n.classList.remove('active'); if (n.dataset.page === 'profile') n.classList.add('active'); }); } else if (lastPage && lastPage.type === 'page' && lastPage.id === 'profile') { document.querySelector('.profile-page').classList.add('active'); document.querySelector('.profile-page').style.display = 'block'; document.querySelectorAll('.nav-item').forEach(n => { n.classList.remove('active'); if (n.dataset.page === 'profile') n.classList.add('active'); }); } else { document.querySelector('.chat-page').classList.add('active'); document.querySelector('.chat-page').style.display = 'block'; loadChats(); document.querySelectorAll('.nav-item').forEach(n => { n.classList.remove('active'); if (n.dataset.page === 'chat') n.classList.add('active'); }); } }, 200); };
...
document.addEventListener('DOMContentLoaded', () => { ensureSinglePage(); setupNavigation(); setupModals(); loadChats(); setupChatListeners(); updateTripsCount(); loadUnreadCounts(); updateUnreadBadge(); });
