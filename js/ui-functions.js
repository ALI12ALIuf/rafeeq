...
const MediaDownloader = {
    ...
    // ✅ sheet تأكيد يظهر رابط حقيقي - المستخدم يضغط عليه مباشرة
    _showSheet(dataUrl, fileName) {
        // إزالة أي sheet قديم
        const old = document.getElementById('_mdl_sheet');
        if (old) old.remove();
        ...
        sheet.style.cssText = `
            ...
            background:#1a1a2e;border-radius:18px 18px 0 0;
            padding:20px;box-shadow:0 -4px 30px rgba(0,0,0,0.6);
            display:flex;flex-direction:column;gap:14px;
            font-family:inherit;
        `;

        const title = document.createElement('div');
        title.textContent = '💾 ' + (fileName || 'تنزيل الملف');
        title.style.cssText = 'color:#fff;font-size:1rem;font-weight:bold;text-align:center;';

        // ✅ رابط حقيقي - المستخدم يضغط عليه مباشرة = user gesture حقيقي
        const link = document.createElement('a');
        link.href = dataUrl;
        link.download = fileName || 'ملف';
        link.textContent = '⬇️ اضغط هنا للتنزيل';
        link.style.cssText = `
            display:block;text-align:center;padding:14px;border-radius:12px;
            background:#4CAF50;color:#fff;font-size:1.05rem;font-weight:bold;
            text-decoration:none;cursor:pointer;
        `;
        // إغلاق الـ sheet بعد الضغط
        link.addEventListener('click', () => setTimeout(() => sheet.remove(), 500));

        const cancel = document.createElement('button');
        cancel.textContent = 'إلغاء';
        cancel.style.cssText = `
            padding:12px;border-radius:12px;border:1px solid #444;
            background:transparent;color:#aaa;font-size:0.95rem;cursor:pointer;
        `;
        cancel.addEventListener('click', () => sheet.remove());

        // إغلاق بالضغط خارج الـ sheet
    ...
    createButton(msgId, fileName, mimeType, dataUrl) {
        ...
        btn.addEventListener('click', function(e) {
            ...
            // ✅ نفتح sheet يحتوي رابط حقيقي - يعمل على Chrome بدون أي قيود
            MediaDownloader._showSheet(btn._dataUrl, btn._fileName);
        });
