...
const MediaDownloader = {
    ...
    // ✅ بناء blob URL مسبقاً عند استلام الملف - قبل أي ضغط
    _buildBlobUrl(dataUrl, mimeType) {
    ...
    createButton(msgId, fileName, mimeType, dataUrl) {
        ...
        // ✅ بناء blob URL الآن - في وقت استلام الملف لا وقت الضغط
        try {
            btn._blobUrl = MediaDownloader._buildBlobUrl(dataUrl, mimeType);
            console.log('✅ blobUrl جاهز:', fileName);
        } catch(e) {
            console.error('❌ فشل بناء blobUrl:', e);
            btn._blobUrl = null;
        }

        // حفظ في IndexedDB للجلسات المستقبلية
        ...
        btn.addEventListener('click', function(e) {
            ...
            if (btn._blobUrl) {
                // ✅ فوري - بدون أي async - Chrome يسمح لأنه في user gesture context مباشر
                ...
                console.log('✅ تنزيل:', btn._fileName);
                btn.innerHTML = '<i class="fas fa-check"></i>';
            } else {
                console.error('❌ blobUrl غير موجود');
                btn.innerHTML = '❌';
            }
