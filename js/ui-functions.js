...
// ==================== نظام التحميل المستقل ====================
// يعمل بشكل مستقل تماماً - كل زر يحمل بياناته الخاصة
const MediaDownloader = {

    // إنشاء زر تحميل مرتبط بالبيانات مباشرة
    createButton(dataUrl, fileName, mimeType) {
        const btn = document.createElement('button');
        btn.className = 'media-dl-btn';
        btn.title = 'تنزيل';
        btn.innerHTML = '<i class="fas fa-download"></i>';

        // ✅ البيانات محفوظة في closure - لا تتأثر بأي شيء خارجي
        const savedData = String(dataUrl);
        const savedName = String(fileName || 'ملف');
        const savedMime = String(mimeType || 'application/octet-stream');
