...
// ✅ دالة تحميل قوية تعمل في كروم وجميع المتصفحات (الحل النهائي)
window.robustDownload = function(url, filename) {
    if (!url) return;
    
    console.log('🚀 بدء التحميل القوي:', filename);
    
    try {
        // الطريقة 1: استخدام رابط مخفي (الأكثر توافقاً مع أسماء الملفات)
        ...
        // ✅ الطريقة 2: (عدوانية لكروم) فتح الرابط في نافذة جديدة إذا لم يبدأ التحميل خلال 300ms
        // هذا يضمن للمستخدم إمكانية التحميل يدوياً من قائمة كروم إذا فشل الزر
        const downloadStarted = false;
        window.addEventListener('blur', () => { downloadStarted = true; }, { once: true });
        
        setTimeout(() => {
            if (link.parentNode) document.body.removeChild(link);
            if (!downloadStarted) {
                console.log('⚠️ لم يكتشف المتصفح تحميل تلقائي، فتح في نافذة جديدة...');
                const newWin = window.open(url, '_blank');
                if (!newWin || newWin.closed || typeof newWin.closed == 'undefined') {
                    alert('يرجى السماح بالنوافذ المنبثقة لتحميل الملف في كروم');
                }
            }
        }, 600);
        
    } catch (e) {
        console.error('❌ فشل التحميل، محاولة الطريقة البديلة:', e);
