...
const DownloadManager = {
    downloadFile(fileData, fileName, fileType) {
        ...
        try {
            // ✅ تنظيف base64 - نزيل كل شيء ليس base64 صحيح
            let cleanData = fileData
                .replace(/[\r\n\t ]/g, '')   // مسافات وسطور
                .replace(/-/g, '+')              // URL-safe base64
                .replace(/_/g, '/');             // URL-safe base64

            // إزالة data URL prefix إذا وُجد
            const base64Match = cleanData.match(/^data:[^;]+;base64,(.+)$/);
            if (base64Match) cleanData = base64Match[1];

            // padding
            const pad = cleanData.length % 4;
            if (pad === 2) cleanData += '==';
            else if (pad === 3) cleanData += '=';

            // ✅ فك base64 مع معالجة الأخطاء
            let binaryString;
            try {
                binaryString = atob(cleanData);
            } catch (e) {
                console.error('❌ atob فشل للملف:', fileName, '| طول البيانات:', cleanData.length);
                this.showNotification('❌ بيانات الملف تالفة: ' + fileName, 'error');
                return false;
            }

            // ✅ تحويل إلى Uint8Array بكفاءة
            ...
            if (blob.size === 0) {
                this.showNotification('❌ الملف فارغ: ' + fileName, 'error');
                return false;
            }

            // ✅ تنزيل مباشر - هذا يجب أن يُستدعى في user-gesture context
            this.downloadBlob(blob, fileName || 'ملف');
            return true;

        } catch (error) {
            console.error('❌ فشل تنزيل الملف:', fileName, error);
            this.showNotification('❌ فشل تنزيل ' + (fileName || 'الملف'), 'error');
            return false;
        }
    },
    
    downloadBlob(blob, filename) {
        // ✅ كل تحميل مستقل - ننشئ URL جديد ونحذفه بعد انتهائه
        ...
        try {
            ...
            // ✅ نُطلق الـ click مباشرة بدون setTimeout (يجب أن يكون في user-gesture context)
            link.click();

            // تنظيف بعد فترة كافية لإكمال التحميل
            setTimeout(() => {
                try { document.body.removeChild(link); } catch(e) {}
                URL.revokeObjectURL(url);
            }, 10000);

            this.showNotification(`✅ جاري تنزيل ${filename} (${sizeKB} KB)`, 'success');

        } catch (error) {
            URL.revokeObjectURL(url);
            console.error('❌ فشل downloadBlob:', error);
            this.showNotification('❌ فشل التنزيل: ' + error.message, 'error');
        }
    },
...
function addDownloadButton(messageElement, fileData, fileName, fileType) {
    ...
    if (fileData.package && fileData.package.data) {
        ...
        // ✅ تخزين نسخة مستقلة من البيانات في الزر (copy by value)
        const _data1 = fileData.package.data ? String(fileData.package.data) : null;
        const _name1 = fileData.package.fileName || fileName;
        const _type1 = fileData.package.fileType || fileType;
        downloadBtn._fileData = { data: _data1, fileName: _name1, fileType: _type1 };
        console.log(`🔗 زر التحميل جاهز: ${_name1} | حجم البيانات: ${_data1 ? _data1.length : 0}`);
        ...
        downloadBtn.onclick = function(e) {
            ...
            const data = btn._fileData;

            // ✅ نُشغّل التحميل مباشرة هنا (في user-gesture context) بدون أي تأخير
            if (data && data.data) {
    ...
    // ✅ تخزين نسخة مستقلة من البيانات في الزر (copy by value)
    const _data2 = fileData.data ? String(fileData.data) : null;
    downloadBtn._fileData = { data: _data2, fileName: fileName, fileType: fileType };
    console.log(`🔗 زر التحميل جاهز: ${fileName} | حجم البيانات: ${_data2 ? _data2.length : 0}`);
