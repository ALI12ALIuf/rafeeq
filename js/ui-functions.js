...
const MediaDownloader = {
    ...
    // ✅ تسجيل SW - بدون await حتى لا يعطل الصفحة
    registerSW() {
        if (!('serviceWorker' in navigator)) return;
        // إلغاء أي SW قديم أولاً ثم تسجيل الجديد
        navigator.serviceWorker.getRegistrations().then(regs => {
            const others = regs.filter(r => !r.scope.includes('/rafeeq/'));
            others.forEach(r => r.unregister());
        });
        navigator.serviceWorker.register('/rafeeq/sw.js', { scope: '/rafeeq/' })
            .then(r => { r.update(); console.log('✅ SW مسجل'); })
            .catch(e => console.warn('⚠️ SW:', e));
    },

    // ✅ إرسال الملف للـ SW فوراً عند الاستلام - قبل أي ضغط
    async storeInSW(id, dataUrl, fileName, mimeType) {
        const sw = navigator.serviceWorker?.controller;
        if (!sw) return null;

        const dlId = 'dl_' + String(id);
        return new Promise((resolve) => {
            const channel = new MessageChannel();
            channel.port1.onmessage = e => {
                if (e.data.type === 'READY') {
                    const url = '/rafeeq/_dl/?id=' + dlId;
                    console.log('✅ SW خزّن الملف:', fileName, '→', url);
                    resolve(url);
                }
            };
            sw.postMessage(
                { type: 'STORE_FILE', id: dlId, fileName, mimeType, data: dataUrl },
                [channel.port2]
            );
            // timeout fallback
    ...
    // ✅ إنشاء الزر - يخزن الملف في SW فوراً ويحفظ الـ URL جاهزاً
    createButton(msgId, fileName, mimeType, dataUrl) {
        ...
        btn._mimeType = mimeType || 'application/octet-stream';
        btn._dataUrl = dataUrl;
        btn._swUrl = null; // سيُملأ بعد تخزين SW

        // حفظ في IndexedDB
        MediaDB.save(String(msgId), dataUrl, fileName, mimeType);

        // ✅ تخزين في SW فوراً - بدون انتظار ضغط المستخدم
        MediaDownloader.storeInSW(msgId, dataUrl, fileName, mimeType).then(url => {
            btn._swUrl = url;
        });
        ...
        btn.addEventListener('click', function(e) {
            ...
            if (btn._swUrl) {
                // ✅ الـ URL جاهز - تنزيل فوري بدون async = Chrome يسمح كل مرة
                a.href = btn._swUrl;
                a.download = btn._fileName;
                console.log('🔽 تنزيل عبر SW:', btn._fileName);
            } else {
                // fallback: blob URL مباشر
                console.warn('⚠️ SW URL غير جاهز، fallback');
                let base64, mime = btn._mimeType;
                let d = btn._dataUrl;
                if (d.startsWith('data:')) {
                    const c = d.indexOf(','); base64 = d.substring(c+1);
                    const m = d.match(/data:([^;]+)/); if(m) mime = m[1];
                } else { base64 = d; }
                ...
                a.href = url; a.download = btn._fileName;
...
// تسجيل SW عند تحميل الصفحة
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => MediaDownloader.registerSW());
} else {
    MediaDownloader.registerSW();
}
