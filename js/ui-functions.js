...
// ==================== MediaDB: تخزين الوسائط في IndexedDB ====================
// يبقى حتى بعد الخروج من المحادثة وإعادة تحميل الصفحة
// يُمسح فقط عند تسجيل الخروج أو انتهاء الجلسة
const MediaDB = {
    _db: null,
    DB_NAME: 'MediaStore',
    STORE: 'media',
    ...
    async save(id, dataUrl, fileName, mimeType) {
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                tx.objectStore(this.STORE).put({ id, dataUrl, fileName, mimeType, ts: Date.now() });
                tx.oncomplete = () => resolve(true);
                tx.onerror = () => reject(tx.error);
            });
        } catch(e) {
            console.warn('MediaDB.save فشل:', e);
            return false;
        }
    },
...
// ==================== MediaDownloader: زر التحميل ====================
const MediaDownloader = {

    // إنشاء زر تحميل - يقرأ البيانات من IndexedDB عند الضغط
    createButton(msgId, fileName, mimeType) {
        const btn = document.createElement('button');
        btn.className = 'media-dl-btn';
        btn.title = 'تنزيل ' + (fileName || 'الملف');
        btn.innerHTML = '<i class="fas fa-download"></i>';
        btn.setAttribute('data-msg-id', msgId);
        ...
        btn.addEventListener('click', async function(e) {
            ...
            try {
                ...
                const record = await MediaDB.get(msgId);
                if (!record || !record.dataUrl) {
                    console.error('❌ البيانات غير موجودة في IndexedDB للـ id:', msgId);
                    btn.innerHTML = '<i class="fas fa-exclamation-triangle"></i>';
                    setTimeout(() => { btn.innerHTML = '<i class="fas fa-download"></i>'; btn._busy = false; }, 2000);
                    return;
                }
                // تحميل مباشر من البيانات المحفوظة
                MediaDownloader.triggerDownload(record.dataUrl, record.fileName, record.mimeType);
    ...
    // تنفيذ التحميل - دائماً عبر Blob URL
    triggerDownload(dataUrl, fileName, mimeType) {
        try {
            ...
            console.error('❌ فشل triggerDownload:', err);
