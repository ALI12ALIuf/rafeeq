// ========== chat-system.js - النسخة النهائية (ثابتة + مشغل فيديو مخصص) ==========
...
const ChatSystem = {
    ...
    // ==================== القسم 26.1: دوال التحكم في الفيديو المخصص ====================
    setupCustomVideoControls(container, video, borderColor) {
        const playBtn = container.querySelector('.custom-video-play-btn');
        const playSmallBtn = container.querySelector('.custom-video-play-btn-small');
        const muteBtn = container.querySelector('.custom-video-mute-btn');
        const progressFill = container.querySelector('.custom-video-progress-fill');
        const progressBar = container.querySelector('.custom-video-progress-bar');
        const currentTimeEl = container.querySelector('.custom-video-current-time');
        const totalTimeEl = container.querySelector('.custom-video-total-time');
        ...
        const togglePlay = () => {
            if (video.paused) {
                ...
                isPlaying = true;
                playBtn.innerHTML = '<i class="fas fa-pause"></i>';
                playSmallBtn.innerHTML = '<i class="fas fa-pause"></i>';
                overlay.style.opacity = '0';
            } else {
                video.pause();
                isPlaying = false;
                playBtn.innerHTML = '<i class="fas fa-play"></i>';
                playSmallBtn.innerHTML = '<i class="fas fa-play"></i>';
                overlay.style.opacity = '1';
            }
        };
        
        playBtn.onclick = (e) => { e.stopPropagation(); togglePlay(); };
        playSmallBtn.onclick = (e) => { e.stopPropagation(); togglePlay(); };
        wrapper.onclick = (e) => { e.stopPropagation(); togglePlay(); };
        ...
        const updateProgressFromMouse = (clientX) => {
        ...
        progressBar.ontouchstart = (e) => {
            ...
            updateProgressFromMouse(touch.clientX);
        };
        ...
        [playBtn, playSmallBtn, muteBtn, progressBar, wrapper].forEach(el => {
            if (el) {
    ...
    displayMessage(msg) {
        ...
        if (msg.type === 'text') {
            ...
            if (currentMessageCount % 10 === 0) {
                const sepTemplate = document.getElementById('timeSeparatorTemplate');
                if (sepTemplate) {
                    const separator = sepTemplate.content.cloneNode(true);
                    const sepDiv = separator.querySelector('.time-separator');
                    if (sepDiv) sepDiv.textContent = dateTime;
        ...
        // ==================== معالجة الصورة ====================
            ...
            if (templateImg) {
                ...
                if (wrapper) {
                    ...
                    const saveBtn = wrapper.querySelector('.media-save-btn');
                    if (saveBtn) {
                        saveBtn.href = msg.data;
                        saveBtn.download = msg.fileName || 'صورة';
        ...
        // ==================== معالجة الفيديو (مشغل مخصص) ====================
            ...
            const templateVid = document.getElementById('customVideoTemplate');
            if (templateVid) {
                const clone = templateVid.content.cloneNode(true);
                const container = clone.querySelector('.custom-video-container');
                if (container) {
                    ...
                    this.setupCustomVideoControls(container, video, borderColor);
                    
                    container.oncontextmenu = (e) => e.preventDefault();
                }
                div.appendChild(clone);
            } else {
                console.warn('⚠️ قالب customVideoTemplate غير موجود');
            }
        }
        ...
        // ==================== معالجة الملف ====================
            ...
            if (templateFile) {
                ...
                if (fileCard) {
                    fileCard.style.background = msg.sender === 'me' ? '#1565C0' : '#4CAF50';
                    ...
                    const saveBtn = fileCard.querySelector('.media-save-btn');
                    if (saveBtn) {
                        saveBtn.href = msg.data;
    ...
    showImagePreview(imageSrc, borderColor = '#4CAF50') {
        ...
        if (saveBtn) { saveBtn.href = imageSrc; saveBtn.style.borderColor = borderColor; }
        this.setupImageZoom(modal, img);
    },

    // ==================== القسم 26.4: setupImageZoom ====================
    ...
    showVideoPreview(videoSrc, borderColor = '#4CAF50') {
        ...
        if (saveBtn) { saveBtn.href = videoSrc; saveBtn.style.borderColor = borderColor; }
    },

    // ==================== القسم 26.6: دوال إغلاق المعاينات ====================
    closeImagePreview() {
        const modal = document.getElementById('imagePreviewModal');
        const img = document.getElementById('previewImage');
