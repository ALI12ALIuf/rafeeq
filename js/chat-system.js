...
openChat(friendId, friendName, friendAvatar) {
    ...
    displayMessages(friendId) { 
        ...
        // ✅ إصلاح: عرض جميع أنواع الرسائل (بما فيها الوسائط المحفوظة في الجلسة)
            ...
            if (wrapper) {
                wrapper.style.border = `2px solid ${borderColor}`;
                    ...
                    if (img) {
                        ...
                        // ✅ إضافة رابط تحميل حقيقي (Anchor Tag) فريد وخاص بالصورة
                        const dlLink = document.createElement('a');
                        dlLink.className = 'unique-dl-btn';
                        dlLink.href = msg.data;
                        dlLink.download = msg.fileName || 'image.jpg';
                        dlLink.innerHTML = '<i class="fas fa-download"></i>';
                        dlLink.style.cssText = 'position: absolute; bottom: 5px; right: 5px; background: rgba(0,0,0,0.5); color: white; border: none; border-radius: 50%; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; text-decoration: none; cursor: pointer; z-index: 10;';
    ...
    // ==================== معالجة الفيديو ====================
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                // ✅ إضافة رابط تحميل حقيقي (Anchor Tag) فريد وخاص بالفيديو
                const dlLink = document.createElement('a');
                dlLink.className = 'unique-dl-btn';
                dlLink.href = msg.data;
                dlLink.download = msg.fileName || 'video.mp4';
                dlLink.innerHTML = '<i class="fas fa-download"></i>';
                dlLink.style.cssText = 'position: absolute; top: 5px; right: 5px; background: rgba(0,0,0,0.5); color: white; border: none; border-radius: 50%; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; text-decoration: none; cursor: pointer; z-index: 10;';
                dlLink.onclick = (e) => e.stopPropagation();
                thumbnail.appendChild(dlLink);
            }
            div.appendChild(clone);
        } else {
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                ...
                if (downloadBtn && msg.data) {
                    // ✅ تحويل الزر إلى رابط حقيقي جاهز للتحميل المباشر
...
downloadPreviewImage() {
    ...
    if (window.robustDownload) {
        window.robustDownload(img.src, 'image.jpg');
    } else {
...
downloadPreviewVideo() {
    ...
    if (window.robustDownload) {
        window.robustDownload(video.src, 'video.mp4');
    } else {
        const link = document.createElement('a');
        link.href = video.src;
...
setupLocationSwipe(locationData) {
    ...
    saveMessage(friendId, message) { 
        // ✅ إصلاح: حفظ جميع الرسائل في الذاكرة (this.messages) للجلسة الحالية
        if (!this.messages[friendId]) this.messages[friendId] = [];
        
        // منع التكرار
        if (!this.messages[friendId].find(m => m.id === message.id)) {
            this.messages[friendId].push(message);
        }

        // حفظ الرسائل النصية فقط في localStorage (لأن Blob URLs لا تعمل بعد التحديث)
        if (message.type === 'text') {
            ...
            if (!localMsgs.find(m => m.id === message.id)) {
                localMsgs.push(message); 
                if (localMsgs.length > 100) localMsgs = localMsgs.slice(-100);
                try { 
...
document.addEventListener('touchmove', function(e) {
    if (document.body.classList.contains('conversation-open')) {
        ...
        // ✅ إصلاح كروم: لا نمنع الحركة على أزرار التحميل
        const isDownloadBtn = e.target.closest('.download-file-btn') ||
                              e.target.closest('[onclick*="downloadPreview"]');
        if (!isMessagesContainer && !isDownloadBtn) {
            e.preventDefault();
        }
    }
}, { passive: false });
...
document.addEventListener('touchend', function (e) {
    ...
    // ✅ إصلاح كروم: لا نمنع النقر على أزرار التحميل والعناصر التفاعلية
    const target = e.target;
    const isDownloadBtn = target.closest('.download-file-btn') || 
                          target.closest('[onclick*="download"]') || 
                          target.closest('[onclick*="Download"]') ||
                          target.closest('a[download]') ||
                          target.tagName === 'A';
    if (now - lastTouchEnd <= 300 && !isDownloadBtn) {
        e.preventDefault();
    }
    lastTouchEnd = now;
}, { passive: false });
