...
init() { 
    ...
    // ✅ تفعيل نظام الضغط المطول لزر تنزيل معاينة الصورة والفيديو
    this.setupImageDownloadLongPress();
    this.setupVideoDownloadLongPress();
},   
    ...
    // ==================== معالجة الفيديو ====================
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                // ✅ منع قائمة المتصفح الافتراضية (تنزيل الفيديو) أثناء الضغط المطول
                if (video) {
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                ...
                if (downloadBtn && msg.data) {
                    // ✅ تنزيل الملف بنظام الضغط المطول بدلاً من الضغطة الواحدة
                    this.attachLongPressDownload(downloadBtn, () => {
...
showImagePreview(imageSrc) {
    ...
    // ✅ منع قائمة المتصفح الافتراضية (حفظ الصورة) أثناء الضغط المطول على الصورة نفسها
...
showVideoPreview(videoSrc) {
    ...
    // ✅ منع قائمة المتصفح الافتراضية (تنزيل الفيديو) أثناء الضغط المطول على الفيديو نفسه
    video.oncontextmenu = (e) => { e.preventDefault(); return false; };
    video.style.webkitTouchCallout = 'none';
...
// ==================== القسم 26.2.4: نظام الضغط المطول للتنزيل (Long-Press Download System) ====================
// ملاحظة: تم إلغاء نظام "الضغطة الواحدة" للتنزيل نهائياً، واستبداله بنظام "الضغط المطول"
// حتى يعمل بشكل موثوق على متصفح كروم، ولمنع ظهور قائمة المتصفح الافتراضية (حفظ/تنزيل)
// أثناء عملية الضغط على الصور والملفات والفيديو.
LONG_PRESS_DURATION: 550, // مدة الضغط المطول المطلوبة بالمللي ثانية

attachLongPressDownload(element, onComplete) {
    if (!element || element._longPressBound) return;
    element._longPressBound = true;
    
    // ✅ منع قائمة المتصفح الافتراضية (كليك يمين / ضغط مطول) على العنصر نفسه
    ...
    const completePress = () => {
        ...
        setTimeout(() => element.classList.remove('long-press-complete'), 300);
        if (navigator.vibrate) { try { navigator.vibrate(15); } catch(e) {} }
        console.log('✅ اكتمل الضغط المطول - تنفيذ التنزيل');
        onComplete();
    };
    ...
    const startPress = (x, y) => {
        ...
        pressTimer = setTimeout(completePress, this.LONG_PRESS_DURATION);
    };
    ...
    // ✅ دعم اللمس (الجوال - كروم): يمنع preventDefault ظهور قائمة الحفظ الافتراضية
    ...
    element.addEventListener('touchcancel', clearPressState, { passive: true });
    
    // ✅ دعم الفأرة (سطح المكتب - كروم)
    element.addEventListener('mousedown', (e) => {
        if (e.button !== 0) return;
        startPress(e.clientX, e.clientY);
    });
    element.addEventListener('mousemove', (e) => movePress(e.clientX, e.clientY));
    element.addEventListener('mouseup', clearPressState);
    element.addEventListener('mouseleave', clearPressState);
    
    // ✅ إلغاء أي تفعيل بالضغطة العادية (كليك) - النظام القديم أصبح معطلاً نهائياً
...
// ==================== القسم 26.2.4.1: ربط زر تنزيل معاينة الصورة بالضغط المطول ====================
setupImageDownloadLongPress() {
    const btn = document.getElementById('downloadImageBtn');
    if (!btn) {
        console.warn('⚠️ زر downloadImageBtn غير موجود');
        return;
    }
    this.attachLongPressDownload(btn, () => this.downloadPreviewImage());
},

// ==================== القسم 26.2.4.2: ربط زر تنزيل معاينة الفيديو بالضغط المطول ====================
setupVideoDownloadLongPress() {
    const btn = document.getElementById('downloadVideoBtn');
    if (!btn) {
        console.warn('⚠️ زر downloadVideoBtn غير موجود');
        return;
    }
    this.attachLongPressDownload(btn, () => this.downloadPreviewVideo());
},
