    ...
    if (msg.type === 'text') {
        ...
        if (templateImg) {
            ...
            if (wrapper) {
                ...
                // ✅ زر تحميل الصورة
                const dlBtn = MediaDownloader.createButton(msg.data, msg.fileName || 'صورة.jpg', 'image/jpeg');
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                // ✅ زر تحميل الفيديو
                const dlBtn = MediaDownloader.createButton(msg.data, msg.fileName || 'فيديو.mp4', 'video/mp4');
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                ...
                const dlBtn = MediaDownloader.createButton(msg.data, msg.fileName || 'ملف', mimeType);
                dlBtn.style.position = 'static';
                dlBtn.style.marginRight = 'auto';
...
showImagePreview(imageSrc) {
    ...
    // ✅ تخزين الـ src للتحميل من الـ modal
    window._previewImageSrc = imageSrc;
    window._downloadPreviewImage = function() {
        if (window._previewImageSrc) {
            MediaDownloader.triggerDownload(window._previewImageSrc, 'صورة_' + Date.now() + '.jpg', 'image/jpeg');
        }
    };
...
showVideoPreview(videoSrc) {
    ...
    // ✅ تخزين الـ src للتحميل من الـ modal
    window._previewVideoSrc = videoSrc;
    window._downloadPreviewVideo = function() {
        if (window._previewVideoSrc) {
            MediaDownloader.triggerDownload(window._previewVideoSrc, 'فيديو_' + Date.now() + '.mp4', 'video/mp4');
        }
    };
},
