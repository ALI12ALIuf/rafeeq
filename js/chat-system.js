    ...
    if (msg.type === 'text') {
        ...
        if (templateImg) {
            ...
            if (wrapper) {
                ...
                const _name = msg.fileName || ('صورة_' + msg.id + '.jpg');
                // ✅ نمرر dataUrl مباشرة - الزر يبني BlobURL فوراً
                MediaDownloader.createButton(msg.id, _name, 'image/jpeg', msg.data).then(btn => {
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                thumbnail.onclick = (e) => { e.stopPropagation(); this.showVideoPreview(msg.data); };
                const _name = msg.fileName || ('فيديو_' + msg.id + '.mp4');
                MediaDownloader.createButton(msg.id, _name, 'video/mp4', msg.data).then(btn => {
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                ...
                const _name = msg.fileName || 'ملف';
                const _mime = msg.fileType || 'application/octet-stream';
                if (fileNameEl) fileNameEl.textContent = _name;
                MediaDownloader.createButton(msg.id, _name, _mime, msg.data).then(btn => {
                    btn.style.position = 'static';
                    btn.style.margin = '6px 0 0 auto';
                    fileCard.appendChild(btn);
                });
            }
            div.appendChild(clone);
        } else {
