    ...
    // ==================== معالجة الفيديو ====================
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                if (video && msg.data) {
                    // ✅ استخدام video.src مباشرة بدل source - يعمل مع كل المتصفحات
                    video.src = msg.data;
                    video.load();
                }
                thumbnail.oncontextmenu = (e) => e.preventDefault();
                thumbnail.onclick = (e) => { e.stopPropagation(); this.showVideoPreview(msg.data, borderColor); };
            }
            div.appendChild(clone);
        } else {
...
showImagePreview(imageSrc, borderColor = '#4CAF50') {
    ...
    if (saveBtn) {
        ...
        saveBtn.download = 'صورة_' + Date.now() + '.jpg';
        saveBtn.style.borderColor = borderColor;
...
showVideoPreview(videoSrc, borderColor = '#4CAF50') {
    ...
    // تطبيق الألوان
    modal.querySelectorAll('.modal-frame').forEach(f => f.style.borderColor = borderColor);
    modal.querySelectorAll('button[onclick*="closeVideo"]').forEach(b => b.style.borderColor = borderColor);
    const saveBtn = document.getElementById('vidPreviewSaveBtn');
    if (saveBtn) {
        saveBtn.href = videoSrc;
        saveBtn.download = 'فيديو_' + Date.now() + '.mp4';
        saveBtn.style.borderColor = borderColor;
        saveBtn.classList.remove('pulse-green','pulse-blue');
        saveBtn.classList.add(borderColor === '#2196F3' ? 'pulse-blue' : 'pulse-green');
    }
    const progress = document.getElementById('vidModalProgress');
    const timeEl = document.getElementById('vidModalTime');
    const playBtn = document.getElementById('vidModalPlayBtn');
    const overlay = document.getElementById('vidModalOverlay');
    const playIcon = document.getElementById('vidModalPlayIcon');
    if (progress) progress.style.accentColor = borderColor;
    if (playBtn) { playBtn.style.borderColor = borderColor; playBtn.style.color = borderColor; }
    ...
    const setPlaying = () => {
        if (playBtn) playBtn.innerHTML = '<i class="fas fa-pause"></i>';
        if (playIcon) playIcon.innerHTML = '<i class="fas fa-pause"></i>';
    ...
    const setEnded = () => {
        if (playBtn) playBtn.innerHTML = '<i class="fas fa-redo"></i>';
        if (playIcon) playIcon.innerHTML = '<i class="fas fa-redo"></i>';
    };

    video.ontimeupdate = updateTime;
    video.onloadedmetadata = () => { if(timeEl) timeEl.textContent = `0:00 / ${fmt(video.duration)}`; };
    video.onplay = setPlaying;
    video.onpause = setPaused;
    video.onended = setEnded;

    window._vidModalToggle = () => {
        if (video.ended) { video.currentTime=0; video.play(); }
        else video.paused ? video.play() : video.pause();
    };
    window._vidModalSeek = (val) => {
