...
init() { 
    ...
    // ✅ تفعيل نظام الضغط المطول لزر تنزيل معاينة الصورة
...
attachLongPressDownload(element, onComplete) {
    ...
    // ✅ Pointer Events تغطي اللمس والفأرة معاً (مثالية لمتصفح كروم)
    element.addEventListener('pointerdown', (e) => {
        e.preventDefault();
        startPress(e.clientX, e.clientY);
    });
    element.addEventListener('pointermove', (e) => movePress(e.clientX, e.clientY));
    element.addEventListener('pointerup', (e) => { e.preventDefault(); cancelPress(); });
    element.addEventListener('pointercancel', cancelPress);
    element.addEventListener('pointerleave', cancelPress);
    
    // ✅ منع ظهور قائمة "حفظ/تنزيل" الافتراضية أثناء اللمس المطول في كروم للجوال
