...
showImagePreview(imageSrc, messageType = 'received') {
    ...
    // تطبيق الألوان بناءً على نوع الرسالة
    this.applyMediaColors(messageType);
...
showVideoPreview(videoSrc, messageType = 'received') {
...
// ==================== القسم 26.4: تطبيق الألوان ديناميكياً ====================
applyMediaColors(messageType = 'received') {
    const color = messageType === 'sent' ? '#2196F3' : '#4CAF50';
    const shadowColor = messageType === 'sent' ? 'rgba(33, 150, 243, 0.3)' : 'rgba(76, 175, 80, 0.3)';
    
    // تحديث لون إطار الصورة
    const imageFrame = document.getElementById('imageFrame');
    if (imageFrame) {
        imageFrame.style.borderColor = color;
        imageFrame.style.boxShadow = `0 0 0 2px ${shadowColor}`;
    }
    
    // تحديث لون أزرار الصورة
    const imageBackBtn = document.getElementById('imageBackBtn');
    const imageDownloadBtn = document.getElementById('imageDownloadBtn');
    if (imageBackBtn) imageBackBtn.style.borderColor = color;
    if (imageDownloadBtn) imageDownloadBtn.style.borderColor = color;
    
    // تحديث لون إطار الفيديو
    const videoFrame = document.getElementById('videoFrame');
    if (videoFrame) {
        videoFrame.style.borderColor = color;
        videoFrame.style.boxShadow = `0 0 0 2px ${shadowColor}`;
    }
    
    // تحديث لون أزرار الفيديو
    const videoBackBtn = document.getElementById('videoBackBtn');
    const videoDownloadBtn = document.getElementById('videoDownloadBtn');
    if (videoBackBtn) videoBackBtn.style.borderColor = color;
    if (videoDownloadBtn) videoDownloadBtn.style.borderColor = color;
    
    // حفظ اللون للاستخدام في الدوال الأخرى
    window._mediaColor = color;
},

// ==================== القسم 26.5: إعداد التحكم بالفيديو ====================
setupVideoControls() {
    const video = document.getElementById('previewVideo');
    ...
    video._controlsSetup = video._controlsSetup || false;
    if (video._controlsSetup) return;
    video._controlsSetup = true;
    ...
    // تحديث الوقت كل 100ms
    video.addEventListener('timeupdate', () => {
        this.updateVideoTime(timeDisplay);
    });
    
    // تحديث الوقت الكلي عند تحميل الفيديو
...
// ==================== القسم 26.6: تبديل تشغيل الفيديو ====================
toggleVideoPlay() {
    const video = document.getElementById('previewVideo');
    if (!video) return;
...
// ==================== القسم 26.8: تحديث عرض الوقت ====================
updateVideoTime(timeDisplay) {
    const video = document.getElementById('previewVideo');
