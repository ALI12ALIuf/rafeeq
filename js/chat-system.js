// ========== chat-system.js - النسخة المعدلة (مع حل مشكلة التحميل في Chrome) ==========
...
const ChatSystem = {
    ...
    // ==================== القسم 26.1: showImagePreview (معدل) ====================
    showImagePreview(imageSrc) {
        const modal = document.getElementById('imagePreviewModal');
        const img = document.getElementById('previewImage');
        if (!modal || !img) return;
        
        // ✅ تحرير الرابط القديم قبل تعيين الرابط الجديد
        if (img.src && img.src.startsWith('blob:')) {
            URL.revokeObjectURL(img.src);
            console.log('🧹 تم تحرير الرابط المؤقت القديم للصورة');
        }
    ...
    // ==================== القسم 26.2: showVideoPreview (معدل) ====================
    showVideoPreview(videoSrc) {
        ...
        if (video.src && video.src.startsWith('blob:')) {
            ...
            console.log('🧹 تم تحرير الرابط المؤقت القديم للفيديو');
        }
    ...
    // ==================== القسم 26.3: دوال إغلاق المعاينات (معدلة) ====================
    closeImagePreview() {
        const modal = document.getElementById('imagePreviewModal');
        const img = document.getElementById('previewImage');
        if (modal) modal.style.display = 'none';
        if (img) { 
            // ✅ تحرير الرابط المؤقت عند الإغلاق
            if (img.src && img.src.startsWith('blob:')) {
                URL.revokeObjectURL(img.src);
                console.log('🧹 تم تحرير الرابط المؤقت للصورة عند الإغلاق');
            }
            img.src = ''; 
    ...
    closeVideoPreview() {
        ...
        if (video) { 
            ...
            if (video.src && video.src.startsWith('blob:')) {
                URL.revokeObjectURL(video.src);
                console.log('🧹 تم تحرير الرابط المؤقت للفيديو عند الإغلاق');
            }
            video.pause(); 
            video.src = ''; 
        }
    ...
    // ==================== القسم 26.4: دوال التحميل (معدلة بالكامل) ====================
    downloadPreviewImage() {
        const img = document.getElementById('previewImage');
        if (!img || !img.src) return;
        
        // ✅ إنشاء عنصر <a> جديد لكل تحميل
        const link = document.createElement('a');
        link.href = img.src;
        ...
        // ✅ تأخير إزالة العنصر لضمان بدء التحميل في Chrome
        setTimeout(() => {
            document.body.removeChild(link);
        }, 100);
        
        console.log('📥 تم بدء تحميل الصورة');
    },
    ...
    downloadPreviewVideo() {
        ...
        console.log('📥 تم بدء تحميل الفيديو');
    },
    ...
    cleanConversationData(chatId, cleanAll = false) {
        ...
        // ✅ تنظيف معاينات الصور والفيديوهات
