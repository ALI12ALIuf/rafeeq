    ...
    if (msg.type === 'text') {
        ...
        if (templateImg) {
            ...
            if (wrapper) {
                ...
                const _name = msg.fileName || ('صورة_' + _id + '.jpg');
                MediaDB.save(_id, msg.data, _name, 'image/jpeg').then(() => {
                    wrapper.appendChild(MediaDownloader.createButton(_id, _name, 'image/jpeg'));
                });
            }
            div.appendChild(clone);
        } else {
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                const _id = String(msg.id);
                const _name = msg.fileName || ('فيديو_' + _id + '.mp4');
                MediaDB.save(_id, msg.data, _name, 'video/mp4').then(() => {
                    thumbnail.appendChild(MediaDownloader.createButton(_id, _name, 'video/mp4'));
                });
            }
            div.appendChild(clone);
        } else {
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                ...
                MediaDB.save(_id, msg.data, _name, _mime).then(() => {
                    const dlBtn = MediaDownloader.createButton(_id, _name, _mime);
