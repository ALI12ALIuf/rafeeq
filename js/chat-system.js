...
// ==================== دالة التحميل الموثوقة (إصلاح مشكلة التحميل المتكرر) ====================
/**
 * دالة تحميل موثوقة تعمل بشكل مستقل لكل ملف
 * - تنشئ عنصر <a> جديد في كل مرة
 * - تضيفه إلى DOM ثم تزيله
 * - تدعم Blob URLs والروابط العادية
 * - تعمل بشكل صحيح في Chrome و Safari و Firefox
 */
function safeDownload(blobUrl, filename) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // إضافة إلى DOM (ضروري لـ Chrome)
            document.body.appendChild(link);

            // محاكاة النقر بشكل صحيح
...
// ==================== دالة تحميل الصورة الموثوقة ====================
function safeDownloadImage(imageSrc, filename) {
    return safeDownload(imageSrc, filename || 'image.jpg');
}

// ==================== دالة تحميل الفيديو الموثوقة ====================
function safeDownloadVideo(videoSrc, filename) {
    return safeDownload(videoSrc, filename || 'video.mp4');
}

// ==================== دالة تحميل الملف الموثوقة ====================
function safeDownloadFile(fileSrc, filename) {
    return safeDownload(fileSrc, filename || 'file');
}
