...
const ChatSystem = {
    ...
    // روابط Blob لوسائط النقل P2P؛ لا تُلغى إلا عند تبديل/إغلاق المحادثة.
    _mediaObjectUrls: new Set(),
    ...
    downloadMedia(sourceUrl, requestedName = 'download') {
        ...
        // إلحاق الرابط بالوثيقة ثم النقر ضمن حدث المستخدم يقلل قيود Chrome على الروابط المؤقتة.
        document.body.appendChild(anchor);
        try {
            anchor.click();
            console.debug('[media-download] started', { name: safeName, source: sourceUrl.slice(0, 32) });
            return true;
        } catch (error) {
            console.error('[media-download] failed', error);
            return false;
        } finally {
            // لا نلغي sourceUrl هنا؛ قد يكون نفس Blob مطلوباً لتنزيل عنصر آخر.
            window.setTimeout(() => anchor.remove(), 1000);
        }
    },

    releaseMediaUrls() {
        this._mediaObjectUrls.forEach((objectUrl) => URL.revokeObjectURL(objectUrl));
...
async requestEnableFeatures() {
    ...
    try {
        ...
        // يجب أن تبقى القناة موثوقة للملفات؛ لا نحدد maxRetransmits كي لا تضيع مقاطع الوسائط.
        CallSystem.dc = CallSystem.pc.createDataChannel('chat', { ordered: true });
            ...
            if (fileCard) {
                ...
                if (downloadBtn && msg.data) {
                    downloadBtn.onclick = (e) => {
                        e.stopPropagation();
                        this.downloadMedia(msg.data, msg.fileName || 'ملف');
                    };
                }
            }
            div.appendChild(clone);
...
downloadPreviewVideo() {
    ...
    this.downloadMedia(video.src, 'video.mp4');
},
...
downloadPreviewImage() {
    ...
    this.downloadMedia(img.src, 'image.jpg');
},
    ...
    async sendImage(file) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                ...
                const tempUrl = this.createMediaUrl(file);
...
// ==================== القسم 33: sendVoiceNote ====================
    async sendVoiceNote(audioBlob) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                const msgId = Date.now().toString();
                const tempUrl = this.createMediaUrl(audioBlob);
...
closeChat() {
    ...
    cleanConversationData(chatId, cleanAll = false) {
        ...
        // هذه هي لحظة انتهاء عمر وسائط P2P؛ لا يتم تحريرها عند تنزيلها أو عند وصول ملف لاحق.
...
function performGlobalCleanup() {
    ...
    if (typeof ChatSystem !== 'undefined') ChatSystem.releaseMediaUrls();
