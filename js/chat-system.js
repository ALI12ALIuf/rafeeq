// ========== chat-system.js - النسخة المعدلة (مع إصلاح التحميل المتكرر) ==========
// نظام الدردشة E2EE + نظام الحضور Presence

// ==================== دوال التحميل الموثوقة (إصلاح مشكلة التحميل المتكرر) ====================
function safeDownload(blobUrl, filename) {
...
function safeDownloadImage(imageSrc, filename) { return safeDownload(imageSrc, filename || 'image.jpg'); }
function safeDownloadVideo(videoSrc, filename) { return safeDownload(videoSrc, filename || 'video.mp4'); }
function safeDownloadFile(fileSrc, filename) { return safeDownload(fileSrc, filename || 'file'); }

// ==================== نهاية دوال التحميل الموثوقة ====================
...
async requestEnableFeatures() {
    ...
    // ✅ 2. انتظر 500ms ثم أرسل الـ Offer
...
async handleFeatureRequest(fromId, encryptedData) {
    ...
    // ✅ معالجة Offer (بعد الإشارة الأولية)
        ...
        if (templateImg) {
            ...
            if (wrapper) {
                ...
                if (img) {
                    ...
                    // ✅ إضافة تحميل بالضغط المطول
                    let pressTimer;
                    const startPress = (e) => {
                        pressTimer = setTimeout(() => {
                            if (msg._blob) {
                                const freshUrl = URL.createObjectURL(msg._blob);
                                safeDownloadImage(freshUrl, msg.fileName || 'image.jpg');
            ...
            if (voiceMsg) {
                ...
                const startPress = (e) => {
                    pressTimer = setTimeout(() => {
                        if (msg._blob) {
                            const freshUrl = URL.createObjectURL(msg._blob);
                            safeDownloadFile(freshUrl, msg.fileName || 'voice.webm');
                            setTimeout(() => URL.revokeObjectURL(freshUrl), 1000);
                        }
                    }, 600);
                };
                const cancelPress = () => clearTimeout(pressTimer);
                voiceMsg.addEventListener('touchstart', startPress);
                voiceMsg.addEventListener('touchend', cancelPress);
                voiceMsg.addEventListener('touchmove', cancelPress);
                voiceMsg.addEventListener('mousedown', startPress);
                voiceMsg.addEventListener('mouseup', cancelPress);
                voiceMsg.addEventListener('mouseleave', cancelPress);
            }
            div.appendChild(clone);
        } else {
            ...
            if (thumbnail) {
                ...
                const startPress = (e) => {
                    pressTimer = setTimeout(() => {
                        if (msg._blob) {
                            const freshUrl = URL.createObjectURL(msg._blob);
                            safeDownloadVideo(freshUrl, msg.fileName || 'video.mp4');
                            setTimeout(() => URL.revokeObjectURL(freshUrl), 1000);
                        }
                    }, 600);
                };
                const cancelPress = () => clearTimeout(pressTimer);
                thumbnail.addEventListener('touchstart', startPress);
                thumbnail.addEventListener('touchend', cancelPress);
                thumbnail.addEventListener('touchmove', cancelPress);
                thumbnail.addEventListener('mousedown', startPress);
                thumbnail.addEventListener('mouseup', cancelPress);
                thumbnail.addEventListener('mouseleave', cancelPress);
            }
            div.appendChild(clone);
        } else {
            ...
            if (fileCard) {
                ...
                if (downloadBtn) {
                    downloadBtn.onclick = (e) => {
                        e.stopPropagation();
                        // ✅ إنشاء URL جديد من الـ Blob في كل مرة
                        if (msg._blob) {
                            const freshUrl = URL.createObjectURL(msg._blob);
                            safeDownloadFile(freshUrl, msg.fileName || 'ملف');
                            setTimeout(() => URL.revokeObjectURL(freshUrl), 1000);
                        } else if (msg.data) {
                            safeDownloadFile(msg.data, msg.fileName || 'ملف');
                        }
                    };
                }
            }
...
downloadPreviewVideo() {
    ...
    // ✅ البحث عن الـ Blob المخزن
    const msgs = ChatSystem.messages[ChatSystem.currentChat] || [];
    const msg = msgs.find(m => m.type === 'video' && m.data === video.src);
    if (msg && msg._blob) {
        const freshUrl = URL.createObjectURL(msg._blob);
        safeDownloadVideo(freshUrl, msg.fileName || 'video.mp4');
        setTimeout(() => URL.revokeObjectURL(freshUrl), 1000);
    } else {
        safeDownloadVideo(video.src, 'video.mp4');
    }
},
...
downloadPreviewImage() {
    ...
    // ✅ البحث عن الـ Blob المخزن
    const msgs = ChatSystem.messages[ChatSystem.currentChat] || [];
    const msg = msgs.find(m => m.type === 'image' && m.data === img.src);
    if (msg && msg._blob) {
        const freshUrl = URL.createObjectURL(msg._blob);
        safeDownloadImage(freshUrl, msg.fileName || 'image.jpg');
        setTimeout(() => URL.revokeObjectURL(freshUrl), 1000);
    } else {
        safeDownloadImage(img.src, 'image.jpg');
    }
},
    ...
    async sendImage(file) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                ...
                this.displayMessage({ id: msgId, type: 'image', data: tempUrl, fileName: file.name, sender: 'me', time: new Date().toISOString(), status: 'sent', _blob: file });
    ...
    async sendFile(file) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                const msgId = Date.now().toString();
                const tempUrl = URL.createObjectURL(file);
                this.displayMessage({ id: msgId, type: 'file', data: tempUrl, fileName: file.name, sender: 'me', time: new Date().toISOString(), status: 'sent', _blob: file });
    ...
    async sendVoiceNote(audioBlob) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                ...
                this.displayMessage({ id: msgId, type: 'voice', data: tempUrl, sender: 'me', time: new Date().toISOString(), status: 'sent', _blob: audioBlob });
