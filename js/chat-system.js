...
// ✅ كشف المتصفح - Chrome يحجب التنزيل المتكرر، باقي المتصفحات لا
const _isChrome = /Chrome/.test(navigator.userAgent) && !/Edg|OPR|SamsungBrowser/.test(navigator.userAgent);
console.log('🌐 المتصفح:', _isChrome ? 'Chrome → ضغط مطول' : 'غير Chrome → ضغطة واحدة');

// ✅ دالة إنشاء زر الحفظ/التنزيل حسب المتصفح
function _createSaveBtn(dataUrl, fileName, borderColor, extraClass = '') {
    const btn = document.createElement('a');
    btn.className = ['media-save-btn', extraClass, borderColor === '#2196F3' ? 'pulse-blue' : 'pulse-green'].filter(Boolean).join(' ');
    btn.style.borderColor = borderColor;

    if (_isChrome) {
        // Chrome: ضغط مطول يفتح قائمة "تنزيل الرابط"
        btn.href = dataUrl;
        btn.innerHTML = '<i class="fas fa-hand-pointer"></i>';
        btn.title = 'اضغط مطولاً للحفظ';
        btn.onclick = (e) => e.preventDefault();
        btn.oncontextmenu = (e) => e.stopPropagation();
    } else {
        // باقي المتصفحات: ضغطة واحدة تنزل مباشرة
        btn.href = '#';
        ...
        btn.onclick = (e) => {
            ...
            } catch(err) { console.error('فشل التنزيل:', err); }
        };
        btn.oncontextmenu = (e) => e.stopPropagation();
    }
    return btn;
...
displayMessage(msg) {
    ...
    // ==================== معالجة الصورة ====================
        ...
        if (templateImg) {
            ...
            if (wrapper) {
                ...
                if (img) {
                    img.src = msg.data;
                    img.onclick = () => this.showImagePreview(msg.data, borderColor, msg.fileName);
    ...
    // ==================== معالجة الفيديو ====================
        ...
        vidWrapper.style.cssText = `border:2px solid ${borderColor};border-radius:14px;overflow:hidden;width:240px;background:#000;position:relative;cursor:pointer;`;
        const thumbVid = document.createElement('video');
        thumbVid.style.cssText = 'width:100%;height:160px;object-fit:cover;display:block;pointer-events:none;';
        thumbVid.preload = 'metadata';
        thumbVid.playsInline = true;
        if (msg.data) { thumbVid.src = msg.data; thumbVid.load(); }
        const playOverlay = document.createElement('div');
        playOverlay.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:44px;height:44px;border-radius:50%;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;color:white;font-size:1.2rem;pointer-events:none;';
        playOverlay.innerHTML = '<i class="fas fa-play"></i>';
        vidWrapper.appendChild(thumbVid);
        vidWrapper.appendChild(playOverlay);
        vidWrapper.oncontextmenu = (e) => e.preventDefault();
        vidWrapper.onclick = (e) => { e.stopPropagation(); this.showVideoPreview(msg.data, borderColor, msg.fileName); };
        const msgContent = document.createElement('div');
        msgContent.className = 'message-content';
        msgContent.style.cssText = 'padding:0;background:transparent;border:none;';
        msgContent.appendChild(vidWrapper);
    ...
    // ==================== معالجة الملف ====================
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                fileCard.style.background = borderColor === '#2196F3' ? '#1565C0' : '#4CAF50';
                fileCard.style.border = `1.5px solid ${borderColor}`;
                const fileNameEl = fileCard.querySelector('.file-name');
                if (fileNameEl) fileNameEl.textContent = msg.fileName || 'ملف';
                fileCard.appendChild(_createSaveBtn(msg.data, msg.fileName, borderColor, 'file-save-btn'));
            }
            div.appendChild(clone);
        } else {
...
showImagePreview(imageSrc, borderColor = '#4CAF50', fileName = 'صورة.jpg') {
    const modal = document.getElementById('imagePreviewModal');
    const img = document.getElementById('previewImage');
    if (!modal || !img) return;
    img.src = imageSrc;
    modal.style.display = 'flex';
    modal.querySelectorAll('.modal-frame').forEach(f => f.style.borderColor = borderColor);
    modal.querySelectorAll('button').forEach(b => b.style.borderColor = borderColor);
    const oldBtn = document.getElementById('imgPreviewSaveBtn');
    if (oldBtn) {
        const newBtn = _createSaveBtn(imageSrc, fileName, borderColor, 'modal-save-btn');
        newBtn.id = 'imgPreviewSaveBtn';
...
showVideoPreview(videoSrc, borderColor = '#4CAF50', fileName = 'فيديو.mp4') {
    ...
    modal.querySelectorAll('.modal-frame').forEach(f => f.style.borderColor = borderColor);
    modal.querySelectorAll('button').forEach(b => b.style.borderColor = borderColor);
    const oldBtn = document.getElementById('vidPreviewSaveBtn');
    if (oldBtn) {
        const newBtn = _createSaveBtn(videoSrc, fileName, borderColor, 'modal-save-btn');
        newBtn.id = 'vidPreviewSaveBtn';
        oldBtn.replaceWith(newBtn);
    }
},
