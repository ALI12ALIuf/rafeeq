// ========== chat-system.js - النسخة المعدلة (مع Service Worker) ==========
...
const ChatSystem = {
    ...
    async disableFeatures() {
        ...
        // ✅ تنظيف ملفات SW
        if (typeof SWManager !== 'undefined') {
            SWManager.clearAllFiles().then(() => {
                console.log('✅ تم تنظيف جميع الملفات من SW');
            }).catch(() => {});
        }
            ...
            if (templateImg) {
                ...
                if (wrapper) {
                    ...
                    if (img) {
                        if (msg.fileUrl) {
                            img.src = msg.fileUrl;
                        } else if (msg._blobUrl) {
                            img.src = msg._blobUrl;
                        }
                        ...
                        img.onclick = () => {
                            ...
                            if (modal && previewImg) {
                                if (msg.fileUrl) {
                                    previewImg.src = msg.fileUrl;
                                } else if (msg._blob) {
                                    const newUrl = URL.createObjectURL(msg._blob);
        ...
        // ==================== الفيديو (معدل مع SW) ====================
            ...
            if (templateVideo) {
                ...
                if (thumbnail) {
                    ...
                    let videoSrc = msg.fileUrl || msg._blobUrl;
                    if (source && videoSrc) {
                        source.src = videoSrc;
                        video.load();
                    }
                ...
                if (voiceMsg) {
                    ...
                    if (audioEl) {
                        if (msg.fileUrl) {
                            ...
                            const audioUrl = URL.createObjectURL(msg._blob);
                    ...
                    if (downloadBtn) {
                        downloadBtn.onclick = (e) => {
                            ...
                            if (msg.fileUrl) {
                                ...
                                link.download = msg.fileName || 'ملف';
                                link.click();
                                setTimeout(() => URL.revokeObjectURL(newUrl), 100);
                            }
                        };
                    }
                }
