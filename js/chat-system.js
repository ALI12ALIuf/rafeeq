...
async kickUserFromConversation() {
    ...
    // ==================== القسم 5.2: زر المعلومات ونافذة السحب ====================
setupInfoButton() {
    // ✅ إضافة أنماط CSS إذا لم تكن موجودة
    if (!document.getElementById('infoButtonStyles')) {
        const style = document.createElement('style');
        ...
        style.textContent = `
            ...
            .swipe-button-info {
                ...
                user-select: none;
                touch-action: none;
                background: linear-gradient(90deg, #1a5a2a 0%, #1a5a2a 50%, #f44336 50%, #f44336 100%);
    ...
    // ✅ إزالة أي زر معلومات قديم
    const oldInfoBtn = document.getElementById('infoBtn');
    if (oldInfoBtn) oldInfoBtn.remove();
    
    const oldInfoModal = document.getElementById('infoSwipeModal');
    if (oldInfoModal) oldInfoModal.remove();
    
    // ✅ إنشاء نافذة المعلومات (Modal)
    const infoModal = document.createElement('div');
    infoModal.id = 'infoSwipeModal';
    infoModal.className = 'info-swipe-modal';
    infoModal.innerHTML = `
        <div class="info-swipe-content">
            <div style="font-size: 3rem; margin-bottom: 10px;">✨</div>
            <h2>ميزات الزر الذهبي</h2>
            <div class="info-list">
                <div class="info-item">🔊 <strong>المكالمات الصوتية والمرئية</strong> - تواصل مباشر بجودة عالية</div>
                <div class="info-item">📷 <strong>الصور والفيديوهات</strong> - إرسال مباشر (الفيديو أقل من 3 دقائق)</div>
                <div class="info-item">📎 <strong>الملفات والبصمات والموقع</strong> - مشاركة فورية وآمنة</div>
                <div class="info-item">🚪 <strong>زر الطرد الأحمر</strong> - إذا أرسلت شيء خطأ، يمكنك طرد المستخدم فوراً والمحتوى لا يُحفظ عنده</div>
                <div class="info-item">🛡️ <strong>الحماية والخصوصية</strong> - بعد الخروج من المحادثة، تختفي الصور والفيديوهات والملفات نهائياً (النصوص فقط تبقى محفوظة في متصفحك)</div>
                <div class="info-item">⚡ <strong>اتصال مباشر P2P</strong> - بدون وسيط، أسرع وأكثر أماناً</div>
            </div>
            <div class="info-note">
                ⚠️ ملاحظة: يجب أن يكون الطرفان في المحادثة لتفعيل الميزات
            </div>
            <div class="swipe-container-info">
                <div id="swipeButtonInfo" class="swipe-button-info">
                    <div class="swipe-label-info">اسحب للإغلاق →</div>
                    <div id="swipeThumbInfo" class="swipe-thumb-info">
    ...
    // ✅ البحث عن حاوية زر التفعيل
    const toggleContainer = document.getElementById('featureToggleContainer');
    if (!toggleContainer) {
        console.log('⚠️ لم يتم العثور على حاوية زر التفعيل');
        return;
    }
    ...
    infoBtn.title = 'معلومات الميزات';
    infoBtn.textContent = '?';
    toggleContainer.appendChild(infoBtn);
    
    // ✅ معالج الضغط لزر المعلومات
    if (infoBtn && infoModal) {
        infoBtn.onclick = () => {
            infoModal.classList.add('show');
            
            const swipeButton = document.getElementById('swipeButtonInfo');
            const swipeThumb = document.getElementById('swipeThumbInfo');
            
            if (!swipeButton || !swipeThumb) return;
            
            let isDragging = false;
            let startX = 0;
            let thumbRight = 8;
            const maxRightMove = swipeButton.offsetWidth - 70;
            
            const onStart = (e) => {
                isDragging = true;
                swipeThumb.style.transition = 'none';
                const clientX = e.type.includes('touch') ? e.touches[0].clientX : e.clientX;
                const rect = swipeThumb.getBoundingClientRect();
                startX = (rect.right - clientX);
            };
            ...
            const onMove = (e) => {
                ...
                e.preventDefault();
                const clientX = e.type.includes('touch') ? e.touches[0].clientX : e.clientX;
                const containerRect = swipeButton.getBoundingClientRect();
                let newRight = (containerRect.right - clientX) - startX;
                newRight = Math.max(8, Math.min(newRight, maxRightMove));
                thumbRight = newRight;
                swipeThumb.style.right = newRight + 'px';
            };
            
            const onEnd = () => {
                if (!isDragging) return;
                isDragging = false;
                swipeThumb.style.transition = 'right 0.3s';
                if (thumbRight >= maxRightMove - 10) {
                    swipeThumb.style.right = maxRightMove + 'px';
                    setTimeout(() => {
                        infoModal.classList.remove('show');
                        setTimeout(() => {
                            swipeThumb.style.right = '8px';
                        }, 300);
                    }, 150);
                } else {
                    swipeThumb.style.right = '8px';
            ...
            infoModal._cleanup = () => {
                swipeThumb.removeEventListener('mousedown', onStart);
                document.removeEventListener('mousemove', onMove);
                document.removeEventListener('mouseup', onEnd);
                swipeThumb.removeEventListener('touchstart', onStart);
            ...
            infoModal.onclick = (e) => {
                if (e.target === infoModal) {
                    if (infoModal._cleanup) infoModal._cleanup();
                    infoModal.classList.remove('show');
                    setTimeout(() => { swipeThumb.style.right = '8px'; }, 300);
                }
            };
        };
    }
    
    console.log('✅ تم إضافة زر المعلومات');
},
...
openChat(friendId, friendName, friendAvatar) {
    ...
    setTimeout(() => this.setupInfoButton(), 600);  // ✅ إضافة زر المعلومات
