            ...
            if (wrapper) {
                wrapper.style.border = `2px solid ${borderColor}`;
                    ...
                    if (img) {
                        ...
                        // ✅ إضافة زر تحميل فريد وخاص بالصورة مباشرة تحتها
                        const dlBtn = document.createElement('button');
                        dlBtn.className = 'unique-dl-btn';
                        dlBtn.innerHTML = '<i class="fas fa-download"></i>';
                        dlBtn.style.cssText = 'position: absolute; bottom: 5px; right: 5px; background: rgba(0,0,0,0.5); color: white; border: none; border-radius: 50%; width: 30px; height: 30px; cursor: pointer; z-index: 10;';
                        dlBtn.onclick = (e) => {
                            e.stopPropagation();
                            if (window.robustDownload) window.robustDownload(msg.data, msg.fileName || 'image.jpg');
                        };
    ...
    // ==================== معالجة الفيديو ====================
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                // ✅ إضافة زر تحميل فريد وخاص بالفيديو مباشرة فوقه
                const dlBtn = document.createElement('button');
                dlBtn.className = 'unique-dl-btn';
                dlBtn.innerHTML = '<i class="fas fa-download"></i>';
                dlBtn.style.cssText = 'position: absolute; top: 5px; right: 5px; background: rgba(0,0,0,0.5); color: white; border: none; border-radius: 50%; width: 30px; height: 30px; cursor: pointer; z-index: 10;';
                dlBtn.onclick = (e) => {
                    e.stopPropagation();
                    if (window.robustDownload) window.robustDownload(msg.data, msg.fileName || 'video.mp4');
                };
                thumbnail.appendChild(dlBtn);
            }
            div.appendChild(clone);
        } else {
            ...
            if (fileCard) {
                ...
                if (downloadBtn && msg.data) {
                    downloadBtn.onclick = (e) => {
                        e.stopPropagation();
                        // ✅ الحل النهائي: استخدام دالة التحميل القوية
                        if (window.robustDownload) {
                            window.robustDownload(msg.data, msg.fileName || 'ملف');
                        } else {
