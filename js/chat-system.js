...
const ChatSystem = {
    ...
    // ✅ تخزين بيانات الوسائط في الذاكرة فقط (لا تُحفظ في localStorage أبداً)
    _mediaStore: {},  // { [msgId]: { data, fileName, fileType, category } }
...
setupLocationSwipe(locationData) {
    ...
    saveMessage(friendId, message) {
        // ✅ الوسائط: تُخزّن في الذاكرة فقط (_mediaStore) - لا تُكتب في localStorage أبداً
        if (message.type !== 'text') {
            if (message._isBase64 && message.id) {
                if (!this._mediaStore[friendId]) this._mediaStore[friendId] = {};
                this._mediaStore[friendId][message.id] = {
                    data: message.data,
                    fileName: message.fileName,
                    fileType: message.fileType,
                    _package: message._package || null
                };
                console.log(`📦 وسائط (${message.type}) مُخزّنة في الذاكرة: ${message.id}`);
            } else {
        ...
        // ✅ النصوص فقط تذهب لـ localStorage
        ...
        if (messages.length > 100) {
            const removeCount = messages.length - 100 + 50;
            messages = messages.slice(removeCount);
            console.log(`🧹 تم حذف ${removeCount} رسالة نصية قديمة`);
        }
        ...
        try { 
            ...
            try { 
                ...
                localStorage.setItem(key, JSON.stringify(messages)); 
                console.log(`🧹 مساحة غير كافية - تم الاحتفاظ بآخر 50 رسالة`);
            } catch (e2) { 
                console.warn('⚠️ تعذّر حفظ الرسائل في localStorage:', e2);
            }
        }
    ...
    // ✅ استرجاع بيانات وسائط من الذاكرة
    getMediaData(friendId, msgId) {
        return (this._mediaStore[friendId] || {})[msgId] || null;
    },

    // ✅ تنظيف وسائط محادثة معينة عند إغلاقها
    clearMediaStore(friendId) {
        if (this._mediaStore[friendId]) {
            delete this._mediaStore[friendId];
            console.log(`🗑️ تم تنظيف وسائط ${friendId} من الذاكرة`);
        }
    },
...
closeChat() {
    ...
    if (chatId) {
        ...
        // ✅ تنظيف وسائط الذاكرة فقط - النصوص في localStorage تبقى سليمة
        this.clearMediaStore(chatId);
        
        // تنظيف الـ src من DOM
        ...
        console.log('✅ تم تنظيف الوسائط من الذاكرة عند إغلاق المحادثة');
    }
