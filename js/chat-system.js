    ...
    if (msg.type === 'text') {
        ...
        if (templateImg) {
            ...
            if (wrapper) {
                ...
                // حفظ في IndexedDB وإضافة زر التحميل
                const imgName = msg.fileName || ('صورة_' + msg.id + '.jpg');
                MediaDB.save(msg.id, msg.data, imgName, 'image/jpeg').then(() => {
                    const dlBtn = MediaDownloader.createButton(msg.id, imgName, 'image/jpeg');
        ...
        if (templateVideo) {
            ...
            if (thumbnail) {
                ...
                // حفظ في IndexedDB وإضافة زر التحميل
                const vidName = msg.fileName || ('فيديو_' + msg.id + '.mp4');
                MediaDB.save(msg.id, msg.data, vidName, 'video/mp4').then(() => {
                    const dlBtn = MediaDownloader.createButton(msg.id, vidName, 'video/mp4');
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                ...
                const fName = msg.fileName || 'ملف';
                const fMime = msg.fileType || 'application/octet-stream';
                if (fileNameEl) fileNameEl.textContent = fName;
                // حفظ في IndexedDB وإضافة زر التحميل
                MediaDB.save(msg.id, msg.data, fName, fMime).then(() => {
                    const dlBtn = MediaDownloader.createButton(msg.id, fName, fMime);
                    dlBtn.style.position = 'static';
                    dlBtn.style.margin = '4px 0 0 auto';
