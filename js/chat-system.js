// ========== chat-system.js - النسخة المعدلة (مع نظام الضغط المطول) ==========
// نظام الدردشة E2EE + نظام الحضور Presence + ضغط مطول للصور والفيديو
...
const ChatSystem = {
    ...
    // ✅ بيانات الميديا الحالية للقائمة
    _currentMediaData: null,
    ...
    displayMessages(friendId) { 
        ...
        messages.forEach(msg => {
            ...
            // ✅ عرض الصور والفيديو أيضاً عند فتح المحادثة
            else if (msg.type === 'image' || msg.type === 'video' || msg.type === 'voice' || msg.type === 'location') {
                this.displayMessage(msg);
            }
        });
    ...
    // ==================== القسم 26: displayMessage (معدل مع الضغط المطول) ====================
    displayMessage(msg) {
        const c = document.getElementById('messagesContainer'); 
        if (!c) return;
        ...
        // ==================== معالجة الصورة (معدل مع الضغط المطول) ====================
            ...
            if (templateImg) {
                ...
                if (wrapper && img) {
                    ...
                    // ✅ حفظ بيانات الصورة للضغط المطول
                    const mediaData = {
                        type: 'image',
                        id: msg.id,
                        url: msg.data,
                        fileName: msg.fileName || 'صورة.jpg',
                        _fileId: msg._fileId || msg.id
                    };
                    
                    // ✅ تطبيق الضغط المطول
                    this.setupLongPressForMedia(img, mediaData);
                    
                    // ✅ إضافة معرف للعنصر
                    img.dataset.mediaId = mediaData._fileId;
                    
                    // ✅ النقر العادي = عرض مكبر
                    img.onclick = (e) => {
                        if (!e._isLongPress) {
        ...
        // ==================== معالجة الفيديو (معدل مع الضغط المطول) ====================
            ...
            if (templateVideo) {
                ...
                if (thumbnail && video && source) {
                    source.src = msg.data;
                    video.load();
                    video.preload = 'metadata';
                    
                    // ✅ حفظ بيانات الفيديو للضغط المطول
                    const mediaData = {
                        type: 'video',
                        id: msg.id,
                        url: msg.data,
                        fileName: msg.fileName || 'فيديو.mp4',
                        _fileId: msg._fileId || msg.id
                    };
                    
                    // ✅ تطبيق الضغط المطول على الفيديو
                    this.setupLongPressForMedia(thumbnail, mediaData);
                    
                    // ✅ إضافة معرف للعنصر
                    thumbnail.dataset.mediaId = mediaData._fileId;
                    ...
                    thumbnail.oncontextmenu = (e) => e.preventDefault();
                }
                div.appendChild(clone);
            } else {
        ...
        // ❌ تم إزالة معالجة الملفات نهائياً
        
        c.appendChild(div); 
        c.scrollTop = c.scrollHeight;
    },

    // ==================== القسم 42: نظام الضغط المطول للصور والفيديو ====================
    
    setupLongPressForMedia(element, mediaData) {
        if (!element) return;
        ...
        const startLongPress = (e) => {
            ...
            longPressTimer = setTimeout(() => {
                ...
                element.style.boxShadow = '0 0 30px rgba(33, 150, 243, 0.3)';
        ...
        const endLongPress = (e) => {
            ...
            // ✅ إذا كان ضغط عادي وليس مطول، نفتح المعاينة
            if (!isLongPress) {
                if (mediaData.type === 'image') {
                    this.showImagePreview(mediaData.url);
                } else if (mediaData.type === 'video') {
                    this.showVideoPreview(mediaData.url);
                }
            }
            isLongPress = false;
            element.style.transform = 'scale(1)';
    ...
    downloadCurrentMedia() {
        if (!this._currentMediaData) return;
        const data = this._currentMediaData;
        const link = document.createElement('a');
        link.href = data.url;
        link.download = data.fileName || (data.type === 'image' ? 'صورة.jpg' : 'فيديو.mp4');
        link.click();
        this.closeContextMenu();
    },
    ...
    copyMediaLink() {
        if (!this._currentMediaData) return;
        const url = this._currentMediaData.url;
    ...
    reloadMedia() {
        ...
        const data = this._currentMediaData;
        if (data._fileId) {
            // محاولة إعادة تحميل من IndexedDB إذا كان مخزناً
            if (typeof FileStorage !== 'undefined' && FileStorage.getFile) {
                FileStorage.getFile(data._fileId).then(record => {
                    if (record) {
                        const newUrl = URL.createObjectURL(record.blob);
                        data.url = newUrl;
                        
                        const elements = document.querySelectorAll(`[data-media-id="${data._fileId}"]`);
                        elements.forEach(el => {
                            if (el.tagName === 'IMG') {
                                el.src = newUrl;
                                ...
                                el.load();
                            } else if (el.classList.contains('video-thumbnail')) {
                        ...
                        this.showToast('✅ تم إعادة تحميل الميديا');
                    } else {
                        this.showToast('❌ الملف غير موجود في الذاكرة');
                    }
                });
            } else {
                this.showToast('❌ ميزة إعادة التحميل غير متوفرة');
            }
        } else {
            this.showToast('❌ لا يمكن إعادة تحميل هذا الملف');
        }
        this.closeContextMenu();
    },
    ...
    // ==================== القسم 26.2.1-5: دوال التحكم بالفيديو ====================
    toggleVideoPlay() {
        const video = document.getElementById('previewVideo');
        const playBtn = document.getElementById('videoPlayBtn');
    ...
    // ==================== القسم 28: sendFileWithRetry (للصور والفيديو فقط) ====================
    ...
    // ==================== القسم 35: saveMessage (معدل - يحفظ جميع الأنواع) ====================
    saveMessage(friendId, message) { 
        ...
        // ✅ حفظ جميع أنواع الرسائل (نص، صورة، فيديو، صوت، موقع)
        const safeMsg = { ...message };
        // إذا كانت وسائط، نحتفظ بالـ URL
        if (safeMsg.type !== 'text' && safeMsg.data) {
            // نحتفظ بالبيانات كما هي
        }
        
        messages.push(safeMsg); 
    ...
    closeChat() {
        ...
        if (chatId) {
            ...
            // ✅ الاحتفاظ بجميع أنواع الرسائل (نص، صورة، فيديو، صوت، موقع)
            // فقط نزيل العناصر التي تحتوي على blob URLs قديمة
            const filteredMessages = messages.filter(msg => {
                if (msg.type === 'text') return true;
                if (msg.type === 'location') return true;
                // للوسائط، نحتفظ بها ولكن نستبدل blob URL إذا كانت منتهية
                if (msg._blobUrl) {
                    // سنقوم بتحديثها عند فتح المحادثة
                }
                return true;
            });
    ...
    cleanConversationData(chatId, cleanAll = false) {
        ...
        if (cleanAll) {
            ...
            const messages = this.messages[chatId] || [];
            // ✅ الاحتفاظ بجميع الأنواع ولكن بحد أقصى 100
            const textMessages = messages.slice(-100);
