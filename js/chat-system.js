// ========== chat-system.js - النسخة المعدلة (مع نظام تخزين ArrayBuffer للتحميل اليدوي) ==========
...
const ChatSystem = {
    ...
    // ✅ نظام تخزين الملفات كـ ArrayBuffer (للتحميل اليدوي)
    ...
    // ==================== القسم 30: sendImage (معدل - إضافة _fileId) ====================
    async sendImage(file) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                ...
                // ✅ تخزين ArrayBuffer في الكاش عند الإرسال
                try {
                ...
                this.displayMessage({ 
                    ...
                    status: 'sent', 
                    _blobUrl: tempUrl,
                    _fileId: msgId  // ✅ ربط الرسالة بالـ ArrayBuffer المخزن
                });
            } else alert('فشل إرسال الصورة');
        }
    },

    // ==================== القسم 31: sendVideoFile (معدل - إضافة _fileId) ====================
    ...
    // ==================== القسم 32: sendFile (معدل - إضافة _fileId) ====================
    async sendFile(file) { 
        if (!this.currentChat) return;
        if (!this.friendInConversation || !this.featuresEnabled) {
            alert(this.featuresEnabled ? 'لا يمكن الإرسال - الطرف الآخر ليس في المحادثة' : 'لا يمكن الإرسال - الميزات غير مفعلة');
    ...
    // ==================== القسم 33: sendVoiceNote (معدل - إضافة _fileId) ====================
    ...
    // ==================== القسم 37: closeChat (معدل - إضافة تنظيف الكاش) ====================
    closeChat() {
    ...
    // ==================== القسم 40: تنظيف بيانات المحادثة (معدل - إضافة تنظيف الكاش) ====================
...
// ==================== القسم 41: التنظيف الشامل عند تحميل الصفحة (معدل - إضافة تنظيف الكاش) ====================
