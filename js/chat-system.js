// ========== chat-system.js - النسخة النهائية (إصلاح التحميل المتكرر) ==========
// نظام الدردشة E2EE + نظام الحضور Presence

// ==================== دوال التحميل الموثوقة (إصلاح نهائي) ====================
/**
 * إصلاح مشكلة التحميل المتكرر في Chrome:
 * - Blob URL يُلغى بعد أول استخدام
 * - الحل: تحويل Blob إلى Base64 Data URL للتحميل
 * - Base64 Data URL يعمل بشكل موثوق ولا يُلغى
 */

// ✅ دالة تحويل Blob إلى Base64
...
// ✅ دالة تحميل موثوقة باستخدام Base64
async function safeDownload(blob, filename, mimeType) {
    try {
        ...
        // إزالة العنصر بعد التحميل (Base64 لا يحتاج revoke)
        setTimeout(() => {
...
// ✅ دوال مساعدة
function safeDownloadImage(blob, filename) { return safeDownload(blob, filename || 'image.jpg', 'image/jpeg'); }
function safeDownloadVideo(blob, filename) { return safeDownload(blob, filename || 'video.mp4', 'video/mp4'); }
function safeDownloadFile(blob, filename) { return safeDownload(blob, filename || 'file', 'application/octet-stream'); }
            ...
            if (fileCard) {
                ...
                if (downloadBtn) {
                    downloadBtn.onclick = async (e) => {
                        e.stopPropagation();
                        // ✅ استخدام safeDownload مع الـ Blob المخزن
                        if (msg._blob) {
                            await safeDownloadFile(msg._blob, msg.fileName || 'ملف');
                        }
                    };
                }
            }
...
downloadPreviewVideo() {
    ...
    if (msg && msg._blob) {
        safeDownloadVideo(msg._blob, msg.fileName || 'video.mp4');
    }
},
...
downloadPreviewImage() {
    ...
    if (msg && msg._blob) {
        safeDownloadImage(msg._blob, msg.fileName || 'image.jpg');
    }
},
