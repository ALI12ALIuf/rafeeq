// ========== chat-system.js - النسخة المعدلة (Base64) ==========
...
const ChatSystem = {
        ...
        // ==================== صورة (Base64) ====================
            ...
            if (templateImg) {
                ...
                if (wrapper) {
                    ...
                    if (img) {
                        img.src = msg.data; // Base64 data:image/jpeg;base64,...
        ...
        // ==================== بصمة صوتية (Base64) ====================
            ...
            if (templateVoice) {
                ...
                if (voiceMsg) {
                    ...
                    if (audioEl && msg.data) {
                        audioEl.src = msg.data; // Base64 data:audio/webm;base64,...
        ...
        // ==================== فيديو (Base64) ====================
            ...
            if (templateVideo) {
                ...
                if (thumbnail) {
                    ...
                    if (source && msg.data) {
                        source.src = msg.data; // Base64 data:video/mp4;base64,...
                        video.load();
                    }
                    thumbnail.onclick = (e) => {
                        e.stopPropagation();
                ...
                if (fileCard) {
                    ...
                    if (downloadBtn && msg.data) {
                        downloadBtn.onclick = (e) => {
                            ...
                            link.href = msg.data; // Base64 data
    ...
    async sendImage(file) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                ...
                try {
                    const base64Data = await SecureChatSystem.compressImageToBase64(file);
                    this.displayMessage({ id: msgId, type: 'image', data: base64Data, fileName: file.name, sender: 'me', time: new Date().toISOString(), status: 'sent', _isBase64: true });
                } catch(e) {
                    console.error('❌ فشل تحويل الصورة إلى Base64:', e);
                }
    ...
    async sendVideoFile(file) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                try {
                    const msgId = Date.now().toString();
                    // تحويل الفيديو إلى Base64 لعرضه
                    const base64Data = await SecureChatSystem.fileToBase64(file);
                    this.displayMessage({ id: msgId, type: 'video', data: base64Data, fileName: file.name, sender: 'me', time: new Date().toISOString(), status: 'sent', _isBase64: true });
                } catch (error) { 
                    console.error('❌ فشل تحويل الفيديو إلى Base64:', error);
                }
    ...
    async sendFile(file) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            ...
            if (success) {
                ...
                try {
                    const base64Data = await SecureChatSystem.fileToBase64(file);
                    this.displayMessage({ id: msgId, type: 'file', data: base64Data, fileName: file.name, sender: 'me', time: new Date().toISOString(), status: 'sent', _isBase64: true });
                } catch(e) {
                    console.error('❌ فشل تحويل الملف إلى Base64:', e);
                }
    ...
    async sendVoiceNote(audioBlob) { 
        ...
        if (CallSystem.dc && CallSystem.dc.readyState === 'open') { 
            // تحويل البصمة إلى Base64
            try {
                const base64Data = await SecureChatSystem.fileToBase64(audioBlob);
                const success = await this.sendFileWithRetry(audioBlob, 'voice');
                if (success) {
                    const msgId = Date.now().toString();
                    this.displayMessage({ id: msgId, type: 'voice', data: base64Data, sender: 'me', time: new Date().toISOString(), status: 'sent', _isBase64: true });
                } else alert('فشل إرسال البصمة الصوتية');
            } catch(e) {
                console.error('❌ فشل تحويل البصمة إلى Base64:', e);
    ...
    // ==================== showLocationSwipeModalWithClicks ====================
