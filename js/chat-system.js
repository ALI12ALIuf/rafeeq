...
openChat(friendId, friendName, friendAvatar) {
  ...
  // ==================== القسم 24: updateFriendStatus (معدل - إلغاء العداد) ====================
updateFriendStatus(friendId, isOnline, userData = null) {
    if (this.currentChat !== friendId) return;
    
    // ✅ تبسيط: نعتمد فقط على انقطاع Data Channel بدلاً من العداد
    // يتم تحديث friendOnline فقط، ولا نبدأ عداد 120 ثانية
    ...
    if (statusEl) {
        if (isOnline) {
            ...
            // إذا كانت الميزات مفعلة، نعرض "غير متصل" مباشرة (بدون عداد)
            if (this.featuresEnabled) {
    ...
    // ✅ إذا كان الطرف الآخر غير متصل والميزات مفعلة، نلغي الميزات فوراً
    if (!isOnline && this.featuresEnabled && this.currentChat === friendId) {
        console.log('🔴 الطرف الآخر غير متصل - إلغاء الميزات فوراً');
