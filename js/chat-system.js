...
displayMessage(msg) {
    ...
    // ==================== معالجة الفيديو ====================
        ...
        const pulseClass = borderColor === '#2196F3' ? 'pulse-blue' : 'pulse-green';

        const vidWrapper = document.createElement('div');
        vidWrapper.className = 'vid-player-wrapper';
        vidWrapper.style.borderColor = borderColor;

        const vid = document.createElement('video');
        vid.className = 'vid-player-el';
        ...
        controls.style.borderTop = `1px solid ${borderColor}44`;

        const playBtn = document.createElement('button');
        playBtn.className = 'vid-play-btn';
        playBtn.style.borderColor = borderColor;
        playBtn.style.color = borderColor;
        playBtn.innerHTML = '<i class="fas fa-play"></i>';
        ...
        progress.style.accentColor = borderColor;

        const expandBtn = document.createElement('button');
        expandBtn.className = 'vid-expand-btn';
        expandBtn.style.color = borderColor;
        expandBtn.innerHTML = '<i class="fas fa-expand"></i>';

        const formatTime = s => { s=Math.floor(s||0); return `${Math.floor(s/60)}:${String(s%60).padStart(2,'0')}`; };

        vid.onloadedmetadata = () => { timeEl.textContent = `0:00 / ${formatTime(vid.duration)}`; };
        vid.ontimeupdate = () => {
            if (!vid.duration) return;
            progress.value = (vid.currentTime / vid.duration) * 100;
            timeEl.textContent = `${formatTime(vid.currentTime)} / ${formatTime(vid.duration)}`;
        };
        vid.onended = () => { playBtn.innerHTML = '<i class="fas fa-redo"></i>'; overlay.style.display = 'flex'; overlay.innerHTML = '<i class="fas fa-redo"></i>'; };
        vid.onplay = () => { playBtn.innerHTML = '<i class="fas fa-pause"></i>'; overlay.style.display = 'none'; };
        vid.onpause = () => { playBtn.innerHTML = '<i class="fas fa-play"></i>'; if (!vid.ended) overlay.style.display = 'flex'; };

        playBtn.onclick = (e) => { e.stopPropagation(); vid.paused ? vid.play() : vid.pause(); };
        overlay.onclick = (e) => { e.stopPropagation(); vid.paused ? vid.play() : vid.pause(); };
        progress.oninput = (e) => { e.stopPropagation(); if (vid.duration) vid.currentTime = (progress.value/100)*vid.duration; };
        progress.onclick = (e) => e.stopPropagation();
        expandBtn.onclick = (e) => { e.stopPropagation(); this.showVideoPreview(msg.data, borderColor); };
        ...
        vidWrapper.appendChild(overlay);
        vidWrapper.appendChild(controls);

        const msgContent = document.createElement('div');
        msgContent.className = 'message-content';
        msgContent.style.cssText = 'padding:0;background:transparent;border:none;overflow:hidden;';
    ...
    // ==================== معالجة الملف ====================
        ...
        if (templateFile) {
            ...
            if (fileCard) {
                ...
                saveBtn.className = `media-save-btn file-save-btn ${borderColor === '#2196F3' ? 'pulse-blue' : 'pulse-green'}`;
                saveBtn.href = msg.data;
                saveBtn.download = msg.fileName || 'ملف';
                saveBtn.innerHTML = '<i class="fas fa-hand-pointer"></i>';
                saveBtn.title = 'اضغط مطولاً للحفظ';
                saveBtn.style.borderColor = borderColor;
                saveBtn.onclick = (e) => e.preventDefault();
                saveBtn.oncontextmenu = (e) => e.stopPropagation();
                fileCard.appendChild(saveBtn);
            }
            div.appendChild(clone);
        } else {
...
showImagePreview(imageSrc, borderColor = '#4CAF50') {
    ...
    modal.querySelectorAll('button').forEach(b => { if(b.style.borderColor !== undefined) b.style.borderColor = borderColor; });
    const saveBtn = document.getElementById('imgPreviewSaveBtn');
    if (saveBtn) {
        saveBtn.href = imageSrc;
        saveBtn.style.borderColor = borderColor;
        saveBtn.classList.remove('pulse-green','pulse-blue');
        saveBtn.classList.add(borderColor === '#2196F3' ? 'pulse-blue' : 'pulse-green');
    }
...
showVideoPreview(videoSrc, borderColor = '#4CAF50') {
    ...
    modal.querySelectorAll('.modal-frame').forEach(f => f.style.borderColor = borderColor);
    modal.querySelectorAll('button').forEach(b => { if(b.style.borderColor !== undefined) b.style.borderColor = borderColor; });
    const saveBtn = document.getElementById('vidPreviewSaveBtn');
    if (saveBtn) {
        saveBtn.href = videoSrc;
        saveBtn.style.borderColor = borderColor;
        saveBtn.classList.remove('pulse-green','pulse-blue');
        saveBtn.classList.add(borderColor === '#2196F3' ? 'pulse-blue' : 'pulse-green');
    }
