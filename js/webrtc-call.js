...
async ensureDataChannelOnly(calleeId) {
    // ✅ منع فتح Data Channel إذا الميزات غير مفعلة (تم إزالة friendInConversation)
...
async receiveCall(callerId, callData) {
    // ✅ إذا كانت الميزات غير مفعلة، لا نستقبل مكالمات
    if (!ChatSystem.featuresEnabled) {
        console.log('❌ الميزات غير مفعلة، لا يمكن استقبال المكالمة');
        this.sendSignal(callerId, { type: 'reject' });
        return;
    }
    
    // ✅ التحقق من أن هذه مكالمة حقيقية (وليست مجرد فتح Data Channel)
    if (callData.sdp && callData.sdp.sdp) {
        const hasAudio = callData.sdp.sdp.includes('m=audio');
        const hasVideo = callData.sdp.sdp.includes('m=video');
        if (!hasAudio && !hasVideo) {
            console.log('📡 هذا ليس مكالمة حقيقية (Data Channel فقط) - تجاهل');
            // معالجة كـ Data Channel فقط
            if (this.pc) {
...
showIncomingCall(callerId, callData) {
    ...
    // ✅ منع عرض شاشة المكالمة إذا كان الـ offer خاصاً بـ Data Channel فقط (بدون صوت/فيديو)
    if (callData.sdp && callData.sdp.sdp) {
        const hasAudio = callData.sdp.sdp.includes('m=audio');
        const hasVideo = callData.sdp.sdp.includes('m=video');
        if (!hasAudio && !hasVideo) {
            console.log('📡 استلام offer خاص بـ Data Channel فقط - لا حاجة لعرض شاشة مكالمة');
            this.handleSignaling(callData);
            return;
        }
    }
