...
async createDataChannelOnly(calleeId) {
    ...
    try {
        ...
        // الملفات لا تستخدم قناة جزئية الموثوقية؛ كل مقطع يجب أن يصل بالترتيب.
    ...
    async receiveCall(callerId, callData) {
        ...
        this.isInCall = true;
        this.callType = 'audio'; // ✅ دائماً صوت
        this.currentCallId = callerId;
        console.log(`📞 استقبال مكالمة صوتية من ${callerId}`);
        ...
        try {
            ...
            const constraints = { audio: true, video: false }; // ✅ صوت فقط
            this.localStream = await navigator.mediaDevices.getUserMedia(constraints);
            ...
            this.showCallUI('audio'); // ✅ دائماً صوت
            ...
            if (callData.sdp) {
                ...
                const answerOptions = { offerToReceiveAudio: true, offerToReceiveVideo: false };
...
showIncomingCall(callerId, callData) {
    ...
    // ✅ دائماً أيقونة هاتف (صوت فقط)
    leftThumb.innerHTML = '<i class="fas fa-phone"></i>';
    ...
    channel.onmessage = e => {
        try {
            if (e.data instanceof ArrayBuffer) {
                this.handleBinaryChunk(e.data);
                return;
            }
            if (e.data instanceof Blob) {
                e.data.arrayBuffer().then(buffer => this.handleBinaryChunk(buffer)).catch(error => {
                    console.error('❌ فشل قراءة جزء ثنائي:', error);
                });
                return;
            }
            ...
            // رسالة قديمة لا تحمل بيانات، ولا ينبغي عرضها كرسالة فارغة في المحادثة.
            if (msg.type === 'file_selection_start') return;
...
async sendSignal(calleeId, data) {
    ...
    const isCallSignal = (data.type === 'audio') || 
...
async sendFileDirect(file, type) {
    // طابور واحد يمنع تداخل مقاطع ملفين عند اختيار وسائط بسرعة.
    const transfer = async () => {
        ...
        try {
            ...
            if (!blobToSend || !blobToSend.size) throw new Error('الملف فارغ أو تعذر تحضيره');

            const channel = this.dc;
            const chunkSize = 16 * 1024;
            const totalChunks = Math.ceil(blobToSend.size / chunkSize);
            const fileId = (window.crypto && typeof window.crypto.randomUUID === 'function')
                ? window.crypto.randomUUID()
                : `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
            const mimeType = blobToSend.type || this.getMimeTypeForTransfer(type);
            const fileName = file.name || this.getFallbackFileName(type);
            const typeLabel = this.getTransferLabel(type);

            // رأس JSON صغير ثم مقاطع ArrayBuffer خام: لا نحول البيانات إلى Array/JSON.
            channel.send(JSON.stringify({
                type: 'file_start', id: fileId, fileType: type, fileName, mimeType,
                size: blobToSend.size, totalChunks
            }));

            console.log(`📤 بدء إرسال ${type}: ${fileName} (${totalChunks} جزء ثنائي)`);
            ChatSystem.showProgressBar(`جاري إرسال ${typeLabel}...`, 0);

            for (let index = 0; index < totalChunks; index++) {
                if (channel.readyState !== 'open') throw new Error('انقطعت قناة البيانات أثناء الإرسال');
                await this.waitForChannelCapacity(channel);
                const start = index * chunkSize;
                const chunk = await blobToSend.slice(start, Math.min(start + chunkSize, blobToSend.size)).arrayBuffer();
                channel.send(chunk);
                ChatSystem.updateProgressBar(((index + 1) / totalChunks) * 100, `جاري إرسال ${typeLabel}...`);
            }

            await this.waitForChannelCapacity(channel);
            channel.send(JSON.stringify({ type: 'file_end', id: fileId, totalChunks, size: blobToSend.size }));
            ChatSystem.hideProgressBar();
            console.log('✅ اكتمل إدخال الملف في قناة البيانات:', fileId);
            return true;
        } catch (error) {
    ...
    const queuedTransfer = this._outgoingFileQueue.then(transfer, transfer);
    this._outgoingFileQueue = queuedTransfer.catch(() => {});
    return queuedTransfer;
},

getTransferLabel(type) {
    return type === 'video' ? 'الفيديو' : type === 'image' ? 'الصورة' : type === 'voice' ? 'البصمة الصوتية' : 'الملف';
},

getMimeTypeForTransfer(type) {
...
getFallbackFileName(type) {
    if (type === 'image') return 'image.jpg';
    if (type === 'video') return 'video.mp4';
    if (type === 'voice') return 'voice.webm';
    return 'file';
},

async waitForChannelCapacity(channel) {
    const maxBufferedAmount = 256 * 1024;
    while (channel.readyState === 'open' && channel.bufferedAmount > maxBufferedAmount) {
        await new Promise(resolve => setTimeout(resolve, 25));
    }
    if (channel.readyState !== 'open') throw new Error('قناة البيانات مغلقة');
},

handleFileStart(msg) {
    const totalChunks = Number(msg.totalChunks);
    const size = Number(msg.size);
    if (!msg.id || !Number.isInteger(totalChunks) || totalChunks < 1 || !Number.isFinite(size) || size < 1) {
        console.warn('⚠️ رأس ملف غير صالح، تم تجاهله');
        return;
    }

    if (this.incomingBinaryTransfer) {
        console.warn('⚠️ نقل وارد سابق لم يكتمل؛ تم إلغاؤه قبل بدء النقل الجديد');
    }

    this.incomingBinaryTransfer = {
        id: msg.id,
        type: msg.fileType || 'file',
        fileName: msg.fileName || this.getFallbackFileName(msg.fileType),
        mimeType: msg.mimeType || this.getMimeTypeForTransfer(msg.fileType),
        size,
        totalChunks,
        receivedChunks: 0,
        receivedBytes: 0,
...
handleBinaryChunk(buffer) {
    const transfer = this.incomingBinaryTransfer;
    if (!transfer) {
        console.warn('⚠️ تم استلام مقطع ثنائي بلا رأس ملف؛ تم تجاهله');
        return;
    }
    ...
    const progress = Math.min(100, (transfer.receivedChunks / transfer.totalChunks) * 100);
    ChatSystem.updateProgressBar(progress, `جاري استلام ${this.getTransferLabel(transfer.type)}...`);
},

handleFileEnd(msg) {
    const transfer = this.incomingBinaryTransfer;
    if (!transfer || msg.id !== transfer.id) {
        console.warn('⚠️ نهاية نقل غير مطابقة؛ تم تجاهلها');
        return;
    }

    const isComplete = transfer.receivedChunks === transfer.totalChunks && transfer.receivedBytes === transfer.size;
    if (!isComplete) {
        console.error('❌ ملف وارد غير مكتمل؛ لن يُعرض كرابط قابل للتنزيل', {
            id: transfer.id, expectedChunks: transfer.totalChunks, receivedChunks: transfer.receivedChunks,
            expectedBytes: transfer.size, receivedBytes: transfer.receivedBytes
        });
        this.incomingBinaryTransfer = null;
        ChatSystem.hideProgressBar();
        return;
    }
    ...
    if (ChatSystem.currentChat) ChatSystem.displayMessage(displayMsg);
    this.incomingBinaryTransfer = null;
    ChatSystem.hideProgressBar();
    console.log('✅ اكتمل استلام ملف صالح للتنزيل:', displayMsg.fileName);
},

// توافق احتياطي مع نقل JSON القديم أثناء الانتقال؛ ستستخدم النسخة الجديدة النقل الثنائي أعلاه.
handleChunkMessage(msg) {
    ...
    if (!this.incomingChunks[msg.id][msg.chunk]) this.incomingFileInfo[msg.id].received++;
    this.incomingChunks[msg.id][msg.chunk] = chunkData;
    const progress = (this.incomingFileInfo[msg.id].received / msg.total) * 100;
    ChatSystem.updateProgressBar(progress, `جاري استلام ${this.getTransferLabel(msg.type)}...`);
    ...
    if (this.incomingFileInfo[msg.id].received === msg.total) {
        ...
        const mimeType = this.getMimeTypeForTransfer(msg.type);
        const objectUrl = URL.createObjectURL(new Blob(chunks, { type: mimeType }));
        const displayMsg = { id: msg.id, type: msg.type, data: objectUrl, fileName: msg.fileName || this.getFallbackFileName(msg.type), sender: 'friend', time: new Date().toISOString(), _blobUrl: objectUrl };
        if (ChatSystem.currentChat) ChatSystem.displayMessage(displayMsg);
        ChatSystem.hideProgressBar();
...
console.log('✅ WebRTC Call System جاهز - مكالمات صوتية فقط');
