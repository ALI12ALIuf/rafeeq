// ========== webrtc-call.js ==========
// نظام اتصال WebRTC - مكالمات صوتية وفيديو + إرسال الملفات والصور
// نسخة متكاملة مع فصل كامل بين المكالمة الصوتية والفيديو

const CallSystem = {
    pc: null, dc: null, localStream: null, isInCall: false, callType: null, callTimerInterval: null,
    ...
    handleCallStatus(msg) {
        if (msg.status === 'connected') console.log('📞 الطرف الآخر متصل');
        else if (msg.status === 'disconnected') {
            console.log('📞 الطرف الآخر قطع الاتصال');
            if (this.isInCall) { alert('الطرف الآخر أنهى المكالمة'); this.endCall(); }
        }
    },
    ...
    applySpeakerSettings() {
        ...
        if (this.remoteAudioElement.setSinkId) {
            if (this.isSpeakerEnabled) {
                this.remoteAudioElement.setSinkId('speaker').catch(e => console.log('فشل التبديل إلى السماعة:', e));
            } else {
    ...
    async receiveCall(callerId, callData) {
        ...
        this.isInCall = true;
        this.callType = callData.type || (callData.sdp?.sdp?.includes('video') ? 'video' : 'audio');
    ...
    showIncomingCall(callerId, callData) {
        ...
        overlay.innerHTML = `
            <div style="text-align:center;">
                ...
                <div style="font-size:1.2rem;margin-top:8px;color:#4CAF50;">${callTypeIcon} ${callTypeText}</div>
            </div>
...
// ==================== الدوال العامة ====================
window.startAudioCall = async () => { if (!ChatSystem.currentChat) return; await CallSystem.startAudioCall(ChatSystem.currentChat); };
window.startVideoCall = async () => { if (!ChatSystem.currentChat) return; await CallSystem.startVideoCall(ChatSystem.currentChat); };
