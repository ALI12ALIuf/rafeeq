...
endCall() {
    ...
    // إعادة فتح القناة إذا كانت الميزات مفعلة
    if (ChatSystem.currentChat && ChatSystem.featuresEnabled) {
        if (this.dc && this.dc.readyState === 'open') {
            ...
            setTimeout(() => {
                this.ensureDataChannelOnly(ChatSystem.currentChat).then(success => {
                    if (success) {
                        console.log('✅ تم إعادة فتح القناة بنجاح بعد المكالمة');
                    } else {
                        console.log('⚠️ فشل إعادة فتح القناة، سيتم إعادة المحاولة لاحقاً');
                        this.scheduleReconnect();
                    }
                });
            }, 500);
