// ========== webrtc-call.js - نسخة المكالمات الصوتية العاملة ==========
...
const CallSystem = {
    ...
    // ========== المكالمة الصوتية - تم إصلاحها بالكامل ==========
    async startCall(calleeId, callType = 'video') {
        ...
        try {
            ...
            // استقبال المسارات البعيدة (الصوت والفيديو)
            this.pc.ontrack = e => { 
                if (e.track.kind === 'video') {
                    const rv = document.getElementById('remoteVideo'); 
                    if (rv && e.streams[0]) rv.srcObject = e.streams[0];
                }
                // الصوت يتم تشغيله تلقائياً، لا حاجة لأي كود إضافي
            };
            ...
            // 🔑 المفتاح: تحديد الخيارات بشكل صحيح حسب نوع المكالمة
            const offerOptions = {
                offerToReceiveAudio: true,  // دائماً true للصوت
                offerToReceiveVideo: (callType === 'video')  // فقط للفيديو
            };
            ...
            this.endCall(); 
            alert('حدث خطأ: ' + (e.message || 'تعذر الوصول إلى الميكروفون/الكاميرا'));
        }
    },
    ...
    // ========== استقبال المكالمة - تم إصلاحها للمكالمات الصوتية ==========
    async receiveCall(callerId, callData) {
        ...
        try {
            // إعداد القيود - صوت فقط للمكالمات الصوتية
            const constraints = { 
                audio: true,
                video: (callType === 'video')
            };
            ...
            if (callData.sdp) { 
                ...
                // 🔑 المفتاح: إنشاء الإجابة مع طلب الصوت فقط للمكالمات الصوتية
                const answerOptions = {
                    offerToReceiveAudio: true,   // دائماً true
            ...
            this.endCall(); 
            alert('حدث خطأ في استقبال المكالمة: ' + (e.message || 'تعذر الوصول إلى الميكروفون/الكاميرا'));
        }
    },
    ...
    showCallUI(callType) { 
        ...
        if (callType === 'audio') {
            ...
            ui.innerHTML = `
                ...
                <div style="position:fixed;bottom:40px;left:50%;transform:translateX(-50%);z-index:9999;display:flex;gap:30px;">
                    <button onclick="CallSystem.switchCamera()" style="width:50px;height:50px;border-radius:50%;background:rgba(0,0,0,0.6);color:white;border:none;font-size:1.2rem;">
                        <i class="fas fa-sync-alt"></i>
                    </button>
                    <button onclick="CallSystem.toggleAudio()" style="width:50px;height:50px;border-radius:50%;background:rgba(0,0,0,0.6);color:white;border:none;font-size:1.2rem;">
                        <i class="fas fa-microphone"></i>
                    </button>
                    <button onclick="CallSystem.endCall()" style="width:60px;height:60px;border-radius:50%;background:#f44336;color:white;border:none;font-size:1.5rem;">
                        <i class="fas fa-phone-slash"></i>
                    </button>
                    <button onclick="CallSystem.toggleVideo()" style="width:50px;height:50px;border-radius:50%;background:rgba(0,0,0,0.6);color:white;border:none;font-size:1.2rem;">
