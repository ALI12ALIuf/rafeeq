...
async sendSignal(calleeId, data) {
    ...
    // ==================== 14. إنهاء المكالمة (نسخة بسيطة) ====================

endCall() {
    console.log('📞 إنهاء المكالمة - الميزات والملفات تبقى نشطة');
    
    // ✅ فقط أغلق MediaStream (الميكروفون والكاميرا)
    if (this.localStream) {
        try {
            this.localStream.getTracks().forEach(t => t.stop());
        } catch(e) {}
    ...
    // ✅ فقط أعد تعيين متغيرات المكالمة
    this.isInCall = false;
    this.callType = null;
    this.currentCallId = null;
    this.isAudioMuted = false;
    ...
    // ❌ لا تغلق this.pc
    // ❌ لا تغلق this.dc
    // ❌ لا توقف keepAliveInterval
    // ❌ لا تلمس reconnectTimer
    // ❌ لا تستدعي cleanupConnections()
    
    console.log('✅ انتهت المكالمة - DataChannel ما زال مفتوحاً للميزات');
    
    // ✅ تحقق من أن القناة ما زالت مفتوحة
    if (this.dc && this.dc.readyState === 'open') {
