...
async createDataChannelOnly(calleeId) {
    ...
    try {
        ...
        // نقل الملفات يحتاج قناة موثوقة؛ maxRetransmits يجعل القناة «جزئية الموثوقية» وقد يُفقد مقاطع في Chrome.
...
async sendFileDirect(file, type) {
    ...
    try {
        ...
        // Date.now وحده قد يتكرر عند بدء نقلين في نفس المللي ثانية.
...
handleChunkMessage(msg) {
    ...
    if (this.incomingFileInfo[msg.id].received === msg.total) {
        ...
        // الاحتفاظ بالـ Blob طوال بقاء المحادثة مفتوحة، حتى يمكن تنزيل كل عنصر لاحقاً.
        const objectUrl = ChatSystem.createMediaUrl(blob);
